...
export default function ChangeManagementPage({ projectPublicId }: { projectPublicId: string }) {
  ...
  const attributes = useMemo(
    () => buildAttributes(enterpriseAttributes, projectAttributes),
  ...
  const recordRows = useMemo<ChangeRecordGridRow[]>(() =>
    records
      .filter((record) => selectedOrderId ? record.change_order_id === selectedOrderId : false)
      .map((record) => ({
        ...record,
  ...
  const orderColumns = useMemo<ColDef<ChangeOrderGridRow>[]>(() => {
    const base: ColDef<ChangeOrderGridRow>[] = [
      ...
      { field: "budget_change", headerName: "Change to Budget", editable: false, type: "numericColumn", minWidth: 135, aggFunc: "sum", valueFormatter: (params) => numberFormat(params.value) },
      { field: "eac_change", headerName: "Change to EAC", editable: false, type: "numericColumn", minWidth: 125, aggFunc: "sum", valueFormatter: (params) => numberFormat(params.value) },
    ];

    const dynamic = attributes.map((attribute): ColDef<ChangeOrderGridRow> => ({
  ...
  const recordColumns = useMemo<ColDef<ChangeRecordGridRow>[]>(() => {
    const base: ColDef<ChangeRecordGridRow>[] = [
    ...
    const dynamic = attributes.map((attribute): ColDef<ChangeRecordGridRow> => ({
      colId: attribute.field,
      headerName: `${attribute.prefix} · ${attribute.columnName}`,
      editable: true,
      minWidth: 145,
  ...
  async function addOrder() {
    ...
    let changeOrderId = `CO-${String(counter).padStart(3, "0")}`;
    while (existing.has(changeOrderId.toLowerCase())) {
      counter += 1;
      changeOrderId = `CO-${String(counter).padStart(3, "0")}`;
    }
