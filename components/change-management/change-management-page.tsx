...
export default function ChangeManagementPage({ projectPublicId, bulkRecords = false }: { projectPublicId: string; bulkRecords?: boolean }) {
  ...
  return <div className="enterprise-admin-page" style={selectedOrder && !bulkRecords ? { position: "fixed", inset: 0, zIndex: 12000, background: "#f5f7fa", display: "flex", flexDirection: "column", padding: 0 } : undefined}>
    ...
    <section className="enterprise-grid-card" style={selectedOrder && !bulkRecords ? { flex: 1, minHeight: 0, display: "flex", flexDirection: "column", border: 0, borderRadius: 0, boxShadow: "none" } : undefined}>
      <div className="enterprise-toolbar" style={selectedOrder && !bulkRecords ? { flexWrap: "wrap", padding: "8px 12px", background: "#fff", borderBottom: "1px solid #e5e7eb" } : { flexWrap: "wrap", alignItems: "center" }}>
        {!selectedOrder && <div style={{ display: "flex", flexDirection: "column", minWidth: 190, marginRight: 4 }}>
          <strong style={{ fontSize: 14, lineHeight: 1.2 }}>{bulkRecords ? "Bulk Change Records" : "Change Orders"}</strong>
          <span style={{ fontSize: 11, color: "#64748b", lineHeight: 1.2 }}>{bulkRecords ? "Manage records across all Change Orders." : "Manage Change Orders and their financial records."}</span>
        </div>}
        {!showingRecords && <button className="button primary" onClick={openNewOrder}>+ Add Change Order</button>}
      ...
      {(!selectedOrder || bulkRecords) && <div className="data-message" style={{ minHeight: 34, paddingTop: 6, paddingBottom: 6 }}><span>Right-click headers for column options. Drag columns to group. Select rows for Bulk Edit/Delete.</span></div>}
      ...
      {!error && !loading && <AgGridProvider modules={[AllEnterpriseModule]} licenseKey={process.env.NEXT_PUBLIC_AG_GRID_LICENSE_KEY ?? ""}><div style={{ height: selectedOrder && !bulkRecords ? "auto" : "calc(100vh - 205px)", flex: selectedOrder && !bulkRecords ? 1 : undefined, minHeight: selectedOrder && !bulkRecords ? 360 : 520, width: "100%", padding: selectedOrder && !bulkRecords ? "8px 12px 10px" : undefined, boxSizing: "border-box" }}>
