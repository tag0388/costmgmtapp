...
import {
  deactivateEnterpriseAttributeValues,
  ENTERPRISE_ATTRIBUTE_SLOTS,
  EnterpriseAttributeCategory,
  EnterpriseAttributeDefinition,
  EnterpriseAttributeValueInput,
  enterpriseAttributeErrorMessage,
  importEnterpriseAttributeValues,
  listEnterpriseAttributes,
  saveEnterpriseAttribute,
} from "@/lib/enterprise-attributes";
...
export default function EnterpriseAttributeDefinitionsPage({ enterprisePublicId, category, pageTitle, description, recordLabel, filenameSegment }: Props) {
  const [enterprise, setEnterprise] = useState<Enterprise | null>(null);
  const [definitions, setDefinitions] = useState<EnterpriseAttributeDefinition[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setDefinitions(await listEnterpriseAttributes(currentEnterprise.id, category));
    } catch (requestError) {
      setError(enterpriseAttributeErrorMessage(requestError));
    } finally {
      setLoading(false);
    }
  }, [category, enterprisePublicId]);
  ...
  return <div className="enterprise-admin-page project-attributes-page">
    ...
    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <div className="attribute-help"><strong>Value List attributes</strong><span>Each slot has a stable number (E01–E20). {recordLabel} records store the Value ID; the UI displays the Value Name.</span></div>
        <button className="button secondary" onClick={() => void refresh()} disabled={loading}>↻ Refresh</button>
      </div>

      {error && <div className="data-message error"><strong>Unable to load {label} attributes</strong><span>{error}</span><button onClick={() => void refresh()}>Try again</button></div>}
      {!error && loading && <div className="data-message"><span className="spinner"/>Loading {label} attributes…</div>}
      {!error && !loading && <div className="enterprise-table-wrap"><table className="enterprise-table attribute-definition-table"><thead><tr><th>Slot</th><th>Attribute Name</th><th>Status</th><th>Allowed Values</th><th>Description</th><th>Actions</th></tr></thead><tbody>{ENTERPRISE_ATTRIBUTE_SLOTS.map((slot) => {
      ...
      <div className="grid-footer"><span>20 stable enterprise {label} attribute slots</span><span>Open an attribute to manage values or use Import / Export</span></div>
    </section>

    {editingSlot && enterprise && <AttributeDrawer enterprise={enterprise} category={category} recordLabel={recordLabel} filenameSegment={filenameSegment} slot={editingSlot} definition={bySlot.get(editingSlot) ?? null} onClose={() => setEditingSlot(null)} onSaved={async () => { setEditingSlot(null); showNotice(`E${String(editingSlot).padStart(2, "0")} ${label} attribute saved.`); await refresh(); }}/>} 
    {notice && <div className="admin-toast" role="status">✓ {notice}</div>}
  </div>;
}

function AttributeDrawer({ enterprise, category, recordLabel, filenameSegment, slot, definition, onClose, onSaved }: { enterprise: Enterprise; category: EnterpriseAttributeCategory; recordLabel: string; filenameSegment: string; slot: number; definition: EnterpriseAttributeDefinition | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const [name, setName] = useState(definition?.name ?? "");
  const [description, setDescription] = useState(definition?.description ?? "");
  const [active, setActive] = useState(definition?.is_active ?? true);
  const [values, setValues] = useState<EnterpriseAttributeValueInput[]>(definition?.attribute_values.filter((value) => value.is_active).map((value) => ({ value_id: value.value_id, value_name: value.value_name })) ?? [{ value_id: "", value_name: "" }]);
  ...
  function updateValue(index: number, field: keyof EnterpriseAttributeValueInput, value: string) {
  ...
  async function save() {
    ...
    setError("");
    try {
      await saveEnterpriseAttribute(enterprise.id, category, slot, {
        name: name.trim(),
  ...
  function exportValues() {
    ...
    exportExcel(`${enterprise.enterprise_code}-${filenameSegment}-E${String(slot).padStart(2, "0")}-${name || "attribute-values"}`, "Values", rows.length ? rows : [{ "Value ID": "", "Value Name": "" }]);
  }
  ...
  async function chooseFile(file: File | undefined) {
    ...
    try {
      ...
      const actualColumns = rows.length ? Object.keys(rows[0]) : [];
      if (rows.length && (!VALUE_COLUMNS.every((column) => actualColumns.includes(column)) || actualColumns.some((column) => !VALUE_COLUMNS.includes(column)))) errors.push("The Excel template must contain exactly these columns: Value ID, Value Name.");
  ...
  async function runImport() {
    ...
    try {
      ...
      await importEnterpriseAttributeValues(definition, incoming, replaceExisting, setProgress);
      setImportRows(null);
      setValues(incoming);
      setProgress(100);
      await onSaved();
    } catch (requestError) {
      setImportErrors([enterpriseAttributeErrorMessage(requestError)]);
    } finally {
      setImporting(false);
    }
  }
  ...
  async function deleteSelectedValues() {
    ...
    if (!window.confirm(`Delete ${selectedValueIds.length} selected value${selectedValueIds.length === 1 ? "" : "s"}? The values will be deactivated and retained for history.`)) return;
    try {
      await deactivateEnterpriseAttributeValues(selectedValueIds);
  ...
  return <><button className="drawer-scrim" onClick={onClose} aria-label="Close attribute editor"/><aside className="admin-drawer attribute-drawer" role="dialog" aria-modal="true" aria-labelledby="attribute-drawer-title"><header><div><span>Enterprise Administration</span><h2 id="attribute-drawer-title">{definition ? "Edit" : "Configure"} {recordLabel} Attribute E{String(slot).padStart(2, "0")}</h2></div><button onClick={onClose} aria-label="Close">×</button></header><div className="drawer-body">{error && <div className="form-error">{error}</div>}<div className="form-grid"><FormField label="Enterprise"><input value={`${enterprise.enterprise_code} — ${enterprise.name}`} readOnly disabled /></FormField><FormField label="Attribute Slot"><input value={`E${String(slot).padStart(2, "0")}`} readOnly disabled /></FormField><FormField label="Attribute Name" required hint={`${name.length}/80`}><input value={name} maxLength={80} onChange={(event) => setName(event.target.value)} autoFocus /></FormField><FormField label="Description"><input value={description} maxLength={160} onChange={(event) => setDescription(event.target.value)} placeholder="Optional description" /></FormField><label className="toggle-field"><span><strong>Active</strong><small>Inactive attributes remain available historically but cannot be newly assigned.</small></span><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)}/><i/></label></div><div className="domains-editor attribute-values-editor"><div className="domains-heading"><div><strong>Allowed Values</strong><span>Value ID is stored on the {recordLabel.toLowerCase()}. Value Name is what users see.</span></div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><button type="button" onClick={exportValues} title="Export values">⇩ Export</button><button type="button" disabled={!definition} onClick={() => fileInput.current?.click()} title={definition ? "Import values" : "Save the attribute before importing values"}>⇧ Import</button><button type="button" disabled={!selectedValueIds.length} onClick={() => void deleteSelectedValues()} title="Delete selected values">🗑 Delete</button><button type="button" onClick={() => setValues((current) => [...current, { value_id: "", value_name: "" }])}>+ Add Value</button></div></div><input ref={fileInput} type="file" accept=".xlsx,.xls" hidden onChange={(event) => void chooseFile(event.target.files?.[0])}/><div className="attribute-values-header"><span></span><span>Value ID</span><span>Value Name</span><span></span></div>{values.map((value, index) => { const persisted = persistedByValueId.get(value.value_id); return <div className="attribute-value-row" key={index} style={{ gridTemplateColumns: "28px 1fr 1.5fr auto" }}><input type="checkbox" aria-label={`Select ${value.value_id || `row ${index + 1}`}`} disabled={!persisted} checked={Boolean(persisted && selectedValueIds.includes(persisted.id))} onChange={(event) => persisted && setSelectedValueIds((current) => event.target.checked ? [...current, persisted.id] : current.filter((id) => id !== persisted.id))}/><input value={value.value_id} maxLength={40} onChange={(event) => updateValue(index, "value_id", event.target.value)} placeholder="e.g. CODE"/><input value={value.value_name} maxLength={80} onChange={(event) => updateValue(index, "value_name", event.target.value)} placeholder="e.g. Display Name"/><button type="button" title="Delete row" onClick={() => setValues((current) => current.filter((_, entryIndex) => entryIndex !== index))}>🗑</button></div>})}{values.length === 0 && <button className="empty-domains" type="button" onClick={() => setValues([{ value_id: "", value_name: "" }])}>+ Add an allowed value</button>}</div></div><footer><button className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving} onClick={() => void save()}>{saving ? "Saving…" : "Save Attribute"}</button></footer></aside>{importRows && <ExcelImportDialog title={`Import ${name || `E${String(slot).padStart(2, "0")}`} values`} rows={importRows} columns={VALUE_COLUMNS} errors={importErrors} replace={replaceExisting} setReplace={setReplaceExisting} importing={importing} progress={progress} replaceLabel="Replace Active Values" replaceDescription="Values not included in the Excel file will become inactive so historical records can still display their original ID and description." onCancel={() => !importing && setImportRows(null)} onImport={() => void runImport()}/>}</>;
}

function FormField({ label, required = false, hint, children }: { label: string; required?: boolean; hint?: string; children: React.ReactNode }) {
