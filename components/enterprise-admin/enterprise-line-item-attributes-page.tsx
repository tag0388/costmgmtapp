...
const CATEGORY = "Line Item" as const;
...
export default function EnterpriseLineItemAttributesPage({ enterprisePublicId }: { enterprisePublicId: string }) {
  ...
  return <div className="enterprise-admin-page project-attributes-page">
    <div className="enterprise-page-title">
      <div>
        <h2>Enterprise Line-Item Attributes</h2>
        <p>Configure up to 20 enterprise-level attributes for cost and commercial line items in {enterprise?.name ?? "this enterprise"}.</p>
      </div>
    ...
    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <div className="attribute-help"><strong>Value List attributes</strong><span>Each slot has a stable number (E01–E20). Line-item records store the Value ID; the UI displays the Value Name.</span></div>
        <button className="button secondary" onClick={() => void refresh()} disabled={loading}>↻ Refresh</button>
      </div>

      {error && <div className="data-message error"><strong>Unable to load line-item attributes</strong><span>{error}</span><button onClick={() => void refresh()}>Try again</button></div>}
      {!error && loading && <div className="data-message"><span className="spinner"/>Loading line-item attributes…</div>}
      ...
      <div className="grid-footer"><span>20 stable enterprise line-item attribute slots</span><span>Open an attribute to manage values or use Excel import/export</span></div>
    </section>

    {editingSlot && enterprise && <AttributeDrawer enterprise={enterprise} slot={editingSlot} definition={bySlot.get(editingSlot) ?? null} onClose={() => setEditingSlot(null)} onSaved={async () => { setEditingSlot(null); showNotice(`E${String(editingSlot).padStart(2, "0")} line-item attribute saved.`); await refresh(); }}/>} 
    {notice && <div className="admin-toast" role="status">✓ {notice}</div>}
  </div>;
}

function AttributeDrawer({ enterprise, slot, definition, onClose, onSaved }: { enterprise: Enterprise; slot: number; definition: EnterpriseAttributeDefinition | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  function exportValues() {
    ...
    exportExcel(`${enterprise.enterprise_code}-line-item-E${String(slot).padStart(2, "0")}-${name || "attribute-values"}`, "Values", rows.length ? rows : [{ "Value ID": "", "Value Name": "" }]);
  }
