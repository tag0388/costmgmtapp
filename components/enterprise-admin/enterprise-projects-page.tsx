...
export default function EnterpriseProjectsPage({ enterprisePublicId }: { enterprisePublicId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const selectedEnterprise = enterprises.find((entry) => entry.public_id === enterprisePublicId) ?? null;
      if (!selectedEnterprise) {
        setEnterprise(null);
        setProjects([]);
        setError("The selected enterprise context is not available.");
        return;
      }
      setEnterprise(selectedEnterprise);
      setProjects(await listProjectsByEnterprise(selectedEnterprise.id));
    } catch (requestError) {
  ...
  return <div className="enterprise-admin-page">
    <div className="enterprise-page-title">
      <div>
        ...
        <p>{enterprise ? `Manage projects within ${enterprise.name}.` : "Manage projects within the selected enterprise."}</p>
      </div>
    ...
    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <label className="enterprise-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search project code or name…" aria-label="Search projects" /></label>
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table"><thead><tr><Sortable label="Project Code" column="project_code" sort={sort} onSort={changeSort}/><Sortable label="Project Name" column="name" sort={sort} onSort={changeSort}/><Sortable label="Status" column="status" sort={sort} onSort={changeSort}/><Sortable label="Created Date" column="created_at" sort={sort} onSort={changeSort}/><th>Route ID</th></tr></thead><tbody>{rows.map((project) => <tr key={project.id} className={selected?.id === project.id ? "selected" : ""} onClick={() => setSelected(project)} onDoubleClick={() => setEditing(project)} tabIndex={0} onKeyDown={(event) => event.key === "Enter" && setEditing(project)}><td className="enterprise-code">{project.project_code}</td><td>{project.name}</td><td><StatusBadge status={project.status}/></td><td>{formatDate(project.created_at)}</td><td className="created-by" title={project.public_id}>{project.public_id}</td></tr>)}</tbody></table></div>}
