...
export default function EnterpriseProjectsPage({ enterprisePublicId }: { enterprisePublicId: string }) {
  ...
  return <div className="enterprise-admin-page enterprise-projects-page">
    <div className="enterprise-page-title">
      ...
      <button className="button primary" onClick={() => setEditing("new")} disabled={!enterprise || loading}>+ Add Project</button>
    </div>

    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <label className="enterprise-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search code, name or routing ID…" aria-label="Search projects" /></label>
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table project-table"><thead><tr>
...
function ProjectDrawer({ enterprise, project, onClose, onSaved }: { enterprise: Enterprise; project: Project | null; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  return <>
    ...
    <aside className="admin-drawer" role="dialog" aria-modal="true" aria-labelledby="project-drawer-title">
      ...
      <div className="drawer-body">
        ...
        <div className="form-grid">
          <FormField label="Enterprise"><input value={`${enterprise.enterprise_code} — ${enterprise.name}`} disabled readOnly/></FormField>
          ...
          <label className="form-field"><span><strong>Status</strong></span><select value={status} onChange={(event) => setStatus(event.target.value as ProjectStatus)}><option value="Active">Active</option><option value="Inactive">Inactive</option></select></label>
          {project && <FormField label="Routing ID" hint="System generated"><input value={project.public_id} disabled readOnly/></FormField>}
        </div>
      </div>
