  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!currentEnterprise) {
        setProjects([]);
        setError("Enterprise context was not found.");
        return;
      }
  ...
  return <div className="enterprise-admin-page">
    <div className="enterprise-page-title">
      <div>
        ...
        <p>{enterprise ? `Manage projects available within ${enterprise.name}.` : "Manage enterprise projects."}</p>
      </div>
    ...
    <section className="enterprise-grid-card">
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table"><thead><tr>
        ...
        <th>Public ID</th>
        <Sortable label="Created" column="created_at" sort={sort} onSort={changeSort}/>
        ...
        <td className="enterprise-code">{project.project_code}</td><td>{project.name}</td><td><StatusBadge status={project.status}/></td><td className="created-by" title={project.public_id}>{project.public_id}</td><td>{formatDate(project.created_at)}</td><td>{formatDate(project.updated_at)}</td>
      </tr>)}</tbody></table></div>}
  ...
  return <><button className="drawer-scrim" onClick={onClose} aria-label="Close project editor"/><aside className="admin-drawer" role="dialog" aria-modal="true" aria-labelledby="project-drawer-title">
    ...
    <div className="drawer-body">
      ...
      <div className="form-grid">
        ...
        <label className="form-field"><span><strong>Status</strong><small>Inactive projects remain available for reporting.</small></span><select value={status} onChange={(event) => setStatus(event.target.value as ProjectStatus)}><option value="Active">Active</option><option value="Inactive">Inactive</option></select></label>
