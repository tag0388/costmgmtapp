...
export default function EnterpriseProjectsPage({ enterprisePublicId }: { enterprisePublicId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setProjects(await listProjectsByEnterprise(selectedEnterprise.id, true));
    } catch (requestError) {
  ...
  function openProject(project: Project) {
    ...
    router.push(`/enterprises/${enterprise.public_id}/projects/${project.public_id}/project-dashboard/overview`);
  }
  ...
  return <div className="enterprise-admin-page">
    ...
    <section className="enterprise-grid-card">
      ...
      <div className="grid-footer"><span>{rows.length} of {projects.length} projects</span><span>Select a row to manage · Double-click to edit</span></div>
    </section>
  ...
  return <><button className="drawer-scrim" onClick={onClose} aria-label="Close project editor"/><aside className="admin-drawer" role="dialog" aria-modal="true" aria-labelledby="project-drawer-title">
    ...
    <div className="drawer-body">
      ...
      <div className="form-grid">
        <label className="form-field"><span><strong>Enterprise</strong><small>Project owner</small></span><input value={`${enterprise.enterprise_code} — ${enterprise.name}`} readOnly disabled /></label>
        ...
        <label className="form-field"><span><strong>Project Name <b>*</b></strong><small>{name.length}/100</small></span><input value={name} maxLength={100} onChange={(event) => setName(event.target.value)} /></label>
        <label className="form-field"><span><strong>Status</strong><small>Inactive projects are retained for reporting.</small></span><select value={status} onChange={(event) => setStatus(event.target.value as ProjectStatus)}><option value="Active">Active</option><option value="Inactive">Inactive</option></select></label>
        {project && <label className="form-field"><span><strong>Public Routing ID</strong><small>Stable system-generated URL identifier</small></span><input value={project.public_id} readOnly disabled /></label>}
      </div>
    </div>
