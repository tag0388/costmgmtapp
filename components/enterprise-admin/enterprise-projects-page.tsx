...
import { createProject, listProjectsByEnterprise, Project, ProjectInput, ProjectStatus, projectErrorMessage, setProjectStatus, updateProject } from "@/lib/projects";
import { isSupabaseConfigured } from "@/lib/supabase/browser";

type StatusFilter = "all" | "active" | "inactive";
type SortKey = "project_code" | "name" | "status" | "created_at" | "created_by";
...
export default function EnterpriseProjectsPage({ enterprisePublicId }: { enterprisePublicId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setError("");
    try {
      const contextEnterprise = await getEnterpriseByPublicId(enterprisePublicId);
      setEnterprise(contextEnterprise);
      if (!contextEnterprise) {
        setProjects([]);
        setError("The selected enterprise could not be found.");
        return;
      }
      setProjects(await listProjectsByEnterprise(contextEnterprise.id, true));
    } catch (requestError) {
  ...
  return <div className="enterprise-admin-page">
    <div className="enterprise-page-title">
      <div>
        ...
        <p>{enterprise ? `Manage projects for ${enterprise.name}.` : "Manage projects for the selected enterprise."}</p>
      </div>
      <button className="button primary" disabled={!enterprise || loading} onClick={() => setEditing("new")}>+ Add Project</button>
    </div>

    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <label className="enterprise-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search code or project name…" aria-label="Search projects" /></label>
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table"><thead><tr><Sortable label="Project Code" column="project_code" sort={sort} onSort={changeSort}/><Sortable label="Project Name" column="name" sort={sort} onSort={changeSort}/><Sortable label="Status" column="status" sort={sort} onSort={changeSort}/><Sortable label="Created Date" column="created_at" sort={sort} onSort={changeSort}/><Sortable label="Created By" column="created_by" sort={sort} onSort={changeSort}/></tr></thead><tbody>{rows.map((project) => <tr key={project.id} className={selected?.id === project.id ? "selected" : ""} onClick={() => setSelected(project)} onDoubleClick={() => setEditing(project)} tabIndex={0} onKeyDown={(event) => event.key === "Enter" && setEditing(project)}><td className="enterprise-code">{project.project_code}</td><td>{project.name}</td><td><StatusBadge status={project.status}/></td><td>{formatDate(project.created_at)}</td><td className="created-by" title={project.created_by ?? "Not recorded"}>{project.created_by ?? "—"}</td></tr>)}</tbody></table></div>}
    ...
    {editing && enterprise && <ProjectDrawer enterprise={enterprise} project={editing === "new" ? null : editing} onClose={() => setEditing(null)} onSaved={async (message) => { setEditing(null); showNotice(message); await refresh(); notifyProjectContextChanged(); }}/>} 
...
function ProjectDrawer({ enterprise, project, onClose, onSaved }: { enterprise:Enterprise; project:Project|null; onClose:()=>void; onSaved:(message:string)=>Promise<void> }) {
  ...
  const [active, setActive] = useState(project?.status !== "Inactive");
  ...
  async function save() {
    ...
    setSaving(true);
    setError("");
    const input: ProjectInput = { project_code: code.trim(), name: name.trim(), status: active ? "Active" : "Inactive" };
    try {
      if (project) await updateProject(project.id, input);
      else await createProject(enterprise.id, input);
  ...
  return <><button className="drawer-scrim" onClick={onClose} aria-label="Close project editor"/><aside className="admin-drawer" role="dialog" aria-modal="true" aria-labelledby="project-drawer-title"><header><div><span>Enterprise Administration</span><h2 id="project-drawer-title">{project ? "Edit Project" : "Add Project"}</h2></div><button onClick={onClose} aria-label="Close">×</button></header><div className="drawer-body">{error && <div className="form-error">{error}</div>}{!isSupabaseConfigured() && <div className="form-warning">Supabase environment variables are not configured in this build.</div>}<div className="form-grid"><FormField label="Enterprise"><input value={`${enterprise.enterprise_code} — ${enterprise.name}`} readOnly disabled /></FormField><FormField label="Project Code" required hint={`${code.length}/20`}><input value={code} maxLength={20} onChange={(event) => setCode(event.target.value)} autoFocus/></FormField><FormField label="Project Name" required hint={`${name.length}/100`}><input value={name} maxLength={100} onChange={(event) => setName(event.target.value)}/></FormField><label className="toggle-field"><span><strong>Active</strong><small>Inactive projects remain available for administration and reporting.</small></span><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)}/><i/></label></div></div><footer><button className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving} onClick={() => void save()}>{saving ? "Saving…" : project ? "Save Changes" : "Add Project"}</button></footer></aside></>;
}
