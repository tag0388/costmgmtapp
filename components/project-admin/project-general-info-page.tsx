...
export default function ProjectGeneralInfoPage({ projectPublicId }: { projectPublicId: string }) {
  ...
  const [form, setForm] = useState({ name: "", status: "Active" as ProjectStatus, start_date: "", finish_date: "", description: "" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setForm({ name: row.name, status: row.status, start_date: row.start_date ?? "", finish_date: row.finish_date ?? "", description: row.description ?? "" });
  ...
  async function save() {
    if (!project) return;
    const name = form.name.trim(); const description = form.description.trim();
  ...
  return <div className="enterprise-admin-page enterprise-settings-page">
    ...
    <section className="enterprise-grid-card enterprise-settings-card">
      <div className="enterprise-settings-section">
        <div className="enterprise-settings-heading"><div><strong>Project Details</strong><span>Project Code and Public ID are fixed identifiers. Other editable fields are stored directly on the Supabase projects table.</span></div></div>
        <div className="form-grid">
          <label className="form-field"><span>Project Code</span><input value={project.project_code} disabled readOnly/></label>
          ...
          <label className="form-field"><span>Public ID</span><input value={project.public_id} disabled readOnly/></label>
