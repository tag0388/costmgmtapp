...
export default function EnterprisesPage() {
  ...
  return <div className="enterprise-admin-page">
    ...
    <section className="enterprise-grid-card">
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table"><thead><tr><Sortable label="Enterprise Code" column="enterprise_code" sort={sort} onSort={changeSort}/><Sortable label="Enterprise Name" column="name" sort={sort} onSort={changeSort}/><Sortable label="Status" column="is_active" sort={sort} onSort={changeSort}/><th>Approved Domains</th><Sortable label="Created Date" column="created_at" sort={sort} onSort={changeSort}/><Sortable label="Created By" column="created_by" sort={sort} onSort={changeSort}/></tr></thead><tbody>{rows.map((enterprise) => <tr key={enterprise.id} className={selected?.id === enterprise.id ? "selected" : ""} onClick={() => setSelected(enterprise)} onDoubleClick={() => setEditing(enterprise)} tabIndex={0} onKeyDown={(event) => event.key === "Enter" && setEditing(enterprise)}><td className="enterprise-code">{enterprise.enterprise_code}</td><td>{enterprise.name}</td><td><StatusBadge active={enterprise.is_active}/></td><td><div className="domain-summary">{enterprise.enterprise_domains?.filter((domain) => domain.is_active).length ? enterprise.enterprise_domains.filter((domain) => domain.is_active).map((domain) => <span key={domain.id}>{domain.domain}</span>) : <em>None</em>}</div></td><td>{formatDate(enterprise.created_at)}</td><td className="created-by" title={enterprise.created_by ?? "Not recorded"}>{enterprise.created_by ?? "—"}</td></tr>)}</tbody></table></div>}
...
function EnterpriseDrawer({ enterprise, onClose, onSaved }: { enterprise:Enterprise|null; onClose:()=>void; onSaved:(message:string)=>Promise<void> }) {
  const [code, setCode] = useState(enterprise?.enterprise_code ?? ""); const [name, setName] = useState(enterprise?.name ?? ""); const [logoUrl, setLogoUrl] = useState(enterprise?.logo_url ?? ""); const [active, setActive] = useState(enterprise?.is_active ?? true);
