...
type SortKey = "enterprise_code" | "name" | "is_active" | "created_at" | "created_by";
...
export default function EnterprisesPage() {
  ...
  const rows = useMemo(() => {
    ...
    return enterprises.filter((enterprise) => {
      const activeDomains = enterprise.enterprise_domains?.filter((domain) => domain.is_active).map((domain) => domain.domain) ?? [];
      ...
      return matchesSearch && (status === "all" || enterprise.is_active === (status === "active"));
    }).sort((left, right) => {
  ...
  async function toggleStatus(enterprise: Enterprise) {
    try { await setEnterpriseActive(enterprise.id, !enterprise.is_active); showNotice(`${enterprise.name} is now ${enterprise.is_active ? "inactive" : "active"}.`); await refresh(); setSelected(null); }
    catch (requestError) { setError(enterpriseErrorMessage(requestError)); }
  ...
  return <div className="enterprise-admin-page">
    ...
    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        ...
        <button className="button secondary" disabled={!selected} onClick={() => selected && (selected.is_active ? setConfirming(selected) : void toggleStatus(selected))}>{selected?.is_active ? "Deactivate" : "Activate"}</button>
      </div>
      ...
      {!error && !loading && rows.length > 0 && <div className="enterprise-table-wrap"><table className="enterprise-table"><thead><tr><Sortable label="Enterprise Code" column="enterprise_code" sort={sort} onSort={changeSort}/><Sortable label="Enterprise Name" column="name" sort={sort} onSort={changeSort}/><Sortable label="Status" column="is_active" sort={sort} onSort={changeSort}/><th>Approved Domains</th><Sortable label="Created Date" column="created_at" sort={sort} onSort={changeSort}/><Sortable label="Created By" column="created_by" sort={sort} onSort={changeSort}/></tr></thead><tbody>{rows.map((enterprise) => <tr key={enterprise.id} className={selected?.id === enterprise.id ? "selected" : ""} onClick={() => setSelected(enterprise)} onDoubleClick={() => setEditing(enterprise)} tabIndex={0} onKeyDown={(event) => event.key === "Enter" && setEditing(enterprise)}><td className="enterprise-code">{enterprise.enterprise_code}</td><td>{enterprise.name}</td><td><StatusBadge isActive={enterprise.is_active}/></td><td><div className="domain-summary">{enterprise.enterprise_domains?.filter((domain) => domain.is_active).length ? enterprise.enterprise_domains.filter((domain) => domain.is_active).map((domain) => <span key={domain.id}>{domain.domain}</span>) : <em>None</em>}</div></td><td>{formatDate(enterprise.created_at)}</td><td className="created-by" title={enterprise.created_by ?? "Not recorded"}>{enterprise.created_by ?? "—"}</td></tr>)}</tbody></table></div>}
...
function StatusBadge({ isActive }: { isActive:boolean }) { return <span className={`enterprise-status ${isActive ? "active" : "inactive"}`}><i/>{isActive ? "Active" : "Inactive"}</span>; }
...
function EnterpriseDrawer({ enterprise, onClose, onSaved }: { enterprise:Enterprise|null; onClose:()=>void; onSaved:(message:string)=>Promise<void> }) {
  const [code, setCode] = useState(enterprise?.enterprise_code ?? ""); const [name, setName] = useState(enterprise?.name ?? ""); const [logoUrl, setLogoUrl] = useState(enterprise?.logo_url ?? ""); const [isActive, setIsActive] = useState(enterprise?.is_active ?? true);
  const [domains, setDomains] = useState<string[]>(enterprise?.enterprise_domains?.filter((domain) => domain.is_active).map((domain) => domain.domain) ?? [""]); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
