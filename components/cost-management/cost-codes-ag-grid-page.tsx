...
export default function CostCodesAgGridPage({ projectPublicId }: { projectPublicId: string }) {
  ...
  function setAllGroupsOpen(open: boolean) {
    ...
    ["cc-general", "cc-settings", "cc-dates", "cc-enterprise", "cc-project", "cc-amounts"].forEach((groupId) => gridApi.setColumnGroupOpened(groupId, open));
  }
...
function CostCodeForm({ projectId, value, enterpriseAttributes, projectAttributes, onClose, onSaved }: { projectId: string; value: CostCode | null; enterpriseAttributes: EnterpriseAttributeDefinition[]; projectAttributes: ProjectAttributeDefinition[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<Form>(() => value ? { cost_code_id: value.cost_code_id, name: value.name, description: value.description, eac_method: value.eac_method, baseline_timephasing_method: value.baseline_timephasing_method, current_budget_timephasing_method: value.current_budget_timephasing_method, ctc_timephasing_method: value.ctc_timephasing_method, manual_eac: value.manual_eac, baseline_start_date: value.baseline_start_date, baseline_finish_date: value.baseline_finish_date, budget_start_date: value.budget_start_date, budget_finish_date: value.budget_finish_date, current_start_date: value.current_start_date, current_finish_date: value.current_finish_date, is_active: value.is_active, ...Object.fromEntries(enterpriseAttributes.map((definition) => [enterpriseField(definition.attribute_number), value[enterpriseField(definition.attribute_number)] ?? null])), ...Object.fromEntries(projectAttributes.map((definition) => [projectField(definition.attribute_number), value[projectField(definition.attribute_number)] ?? null])) } as Form : blankForm);
