...
function SvgIcon({ type }: { type: "changes" | "actual" | "ctc" | "more" | "resource" | "add" }) {
  ...
  if (type === "resource") return <svg {...common}><path d="M4 5h16v14H4z"/><path d="M8 9h8"/><path d="M8 13h5"/></svg>;
  if (type === "add") return <svg {...common}><path d="M12 5v14"/><path d="M5 12h14"/></svg>;
...
function periodLabel(period: CostReportingPeriod) { return `P${period.period_number} | ${longDate(period.end_date)}`; }
function periodExcel(period: CostReportingPeriod) { return `P${period.period_number} | ${shortDate(period.end_date)}`; }
function periodLegacy(period: CostReportingPeriod) { return `P${period.period_number}`; }
...
function PeriodHeader({ period }: { period: CostReportingPeriod }) {
  return <div style={{ width: "100%", textAlign: "center", lineHeight: "14px", paddingTop: 2 }}><strong style={{ display: "block", fontSize: 11 }}>P{period.period_number}</strong><span style={{ display: "block", fontSize: 9.5, color: "#667085", whiteSpace: "nowrap" }}>{shortDate(period.end_date)}</span></div>;
}
...
function RelatedRecordsWorkspace({ project, costCode, mode, onClose }: { project: Project; costCode: CostCode; mode: RelatedMode; onClose: () => void }) {
  ...
  const periodByLabel = useMemo(() => new Map(periods.flatMap((period) => [[periodLabel(period).toUpperCase(), period], [periodExcel(period).toUpperCase(), period], [periodLegacy(period).toUpperCase(), period]] as const)), [periods]);
  ...
  const enterpriseResourceMap = useMemo(() => new Map(activeEnterpriseResources.map((row) => [row.resource_id.toLowerCase(), row])), [activeEnterpriseResources]);
  const projectResourceMap = useMemo(() => new Map(activeProjectResources.map((row) => [row.resource_id.toLowerCase(), row])), [activeProjectResources]);
  ...
  const pickableResources = useMemo<PickableResource[]>(() => {
    const source = resourceScope === "enterprise" ? activeEnterpriseResources.map((row) => ({ scope: "enterprise" as const, ...row })) : activeProjectResources.map((row) => ({ scope: "project" as const, ...row }));
    const q = resourceSearch.trim().toLowerCase();
    return source.filter((row) => !q || row.resource_id.toLowerCase().includes(q) || row.resource_name.toLowerCase().includes(q) || row.category.toLowerCase().includes(q));
  ...
  function selectionChanged(event: SelectionChangedEvent<RelatedGridRow>) {
    ...
    if (selected[0]?.id) setFocusedRowId(selected[0].id);
  }
  function insertIndex<T extends { id: string }>(current: T[]) {
    if (!focusedRowId) return current.length;
    const index = current.findIndex((row) => row.id === focusedRowId);
  ...
  const lockedStyle = useCallback((params: { data?: CtcGridRow }) => params.data?.resource_source ? { backgroundColor: "#f5f6f8", color: "#667085" } : undefined, []);
  ...
  const ctcColumnDefs = useMemo<ColDef<CtcGridRow>[]>(() => [
    { field: "resource_source_label", headerName: "Resource Source", editable: false, width: 105, maxWidth: 120, filter: "agSetColumnFilter" },
    { field: "item", headerName: "Item", editable: (params) => !params.data?.resource_source, filter: true, cellStyle: lockedStyle },
    ...
    { colId: "qty", headerName: "Qty", editable: false, type: "numericColumn", aggFunc: "sum", enableValue: true, valueGetter: (params) => Object.values(params.data?.period_qty ?? {}).reduce((sum, value) => sum + Number(value || 0), 0), valueFormatter: (params) => numberFormat(params.value, 4), width: 90 },
    { field: "unit", headerName: "Unit", editable: (params) => !params.data?.resource_source, filter: "agSetColumnFilter", width: 90, cellStyle: lockedStyle },
    { field: "rate", headerName: "Rate", editable: (params) => !params.data?.resource_source, type: "numericColumn", valueParser: (params) => Number(params.newValue), valueFormatter: (params) => numberFormat(params.value, 4), width: 100, cellStyle: lockedStyle },
    { colId: "total", headerName: "Total", editable: false, type: "numericColumn", aggFunc: "sum", enableValue: true, valueGetter: (params) => Object.values(params.data?.period_qty ?? {}).reduce((sum, value) => sum + Number(value || 0), 0) * Number(params.data?.rate ?? 0), valueFormatter: (params) => numberFormat(params.value, 2), width: 110 },
    { field: "category", headerName: "Category", editable: true, cellEditor: "agSelectCellEditor", cellEditorParams: { values: ["", ...RESOURCE_CATEGORIES] }, filter: "agSetColumnFilter" },
  ...
  ], [attributeEditor, ctcAttributes, lockedStyle, periods]);
  ...
  async function actualChanged(event: CellValueChangedEvent<ActualGridRow>) {
    ...
    try {
      ...
      if (colId === "period_label") { const period = periodByLabel.get(String(event.newValue).toUpperCase()); if (!period) throw new Error("Select a valid Cost Reporting Period."); await updateActualCostTransaction(event.data.id, { cost_period_id: period.id, transaction_date: period.end_date }); }
      else if (colId === "amount") await updateActualCostTransaction(event.data.id, { amount: Number(event.newValue) });
  ...
  async function ctcChanged(event: CellValueChangedEvent<CtcGridRow>) {
    ...
    try {
      const colId = event.column.getColId();
      if (colId.startsWith("period:")) { const periodId = colId.slice(7); const qty = Number(event.data.period_qty[periodId] ?? 0); await setCostToCompletePeriodQty(event.data.id, periodId, qty); event.api.refreshCells({ rowNodes: [event.node], columns: ["qty", "total"], force: true }); }
      else if (colId === "item") await updateCostToCompleteDetail(event.data.id, { item: event.newValue ? String(event.newValue).trim() : null });
  ...
  async function addRows() {
    ...
    try {
      if (mode === "actual") {
        ...
        for (let i = 0; i < count; i += 1) created.push(await createActualCostTransaction(project.id, { cost_period_id: period.id, cost_code_id: costCode.id, transaction_date: period.end_date, transaction_id: null, description: "", amount: 0, transaction_type: "MAN", ...Object.fromEntries(actualAttributes.map((a) => [a.field, null])) }));
        setActualRows((current) => { const index = insertIndex(current); return [...current.slice(0, index), ...created, ...current.slice(index)]; });
        setFocusedRowId(created[created.length - 1]?.id ?? focusedRowId);
      } else {
        const created: CostToCompleteLedgerRow[] = [];
        for (let i = 0; i < count; i += 1) created.push(await createCostToCompleteDetail(project.id, { cost_code_id: costCode.id, item: null, description: null, unit: null, rate: 0, category: null, resource_source: null, ...Object.fromEntries(ctcAttributes.map((a) => [a.field, null])) }));
        setCtcRows((current) => { const index = insertIndex(current); return [...current.slice(0, index), ...created, ...current.slice(index)]; });
        setFocusedRowId(created[created.length - 1]?.id ?? focusedRowId);
      }
      showNotice(`${count} row${count === 1 ? "" : "s"} added${focusedRowId ? " below the selected row" : ""}.`);
    } catch (requestError) { setError(mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError)); }
    finally { setSaving(false); }
  }

  async function addResource(resource: PickableResource) {
    setSaving(true); setError("");
    try {
      const row = await createCostToCompleteDetail(project.id, {
        cost_code_id: costCode.id,
        resource_source: resource.scope === "enterprise" ? "ERes" : "PRes",
      ...
      setCtcRows((current) => { const index = insertIndex(current); return [...current.slice(0, index), row, ...current.slice(index)]; });
      setFocusedRowId(row.id);
      showNotice(`${resource.resource_id} added below the selected row.`);
  ...
  function normalizeImportRows(incoming: ExcelRow[]) {
    return incoming.map((row) => {
      const next = { ...row };
      periods.forEach((period) => {
        const legacy = periodLegacy(period);
        const current = periodExcel(period);
        if (next[legacy] !== undefined && next[current] === undefined) { next[current] = next[legacy]; delete next[legacy]; }
      });
      return next;
    });
  }
  ...
  async function chooseImport(file: File | undefined) {
    if (!file) return;
    try {
      const incoming = normalizeImportRows(await readExcel(file));
      ...
      incoming.forEach((row, index) => {
        ...
        if (mode === "actual") {
          ...
          if (!periodByLabel.has((row["Cost Reporting Period"] ?? "").trim().toUpperCase())) errors.push(`Row ${line}: Cost Reporting Period must match a valid period.`);
        } else {
          ...
          if (sourceText === "ERes" && !enterpriseResourceMap.has(item.toLowerCase())) errors.push(`Row ${line}: Item must be an active Enterprise Resource ID for ERes.`);
          if (sourceText === "PRes" && !projectResourceMap.has(item.toLowerCase())) errors.push(`Row ${line}: Item must be an active Project Resource ID for PRes.`);
  ...
  async function runImport() {
    ...
    try {
      ...
      for (let index = 0; index < importRows.length; index += 1) {
        ...
        if (mode === "actual") {
          const period = periodByLabel.get((row["Cost Reporting Period"] ?? "").trim().toUpperCase())!;
          ...
          const sourceText = (row["Resource Source"] ?? "").trim();
          const item = (row.Item ?? "").trim();
          const resource = sourceText === "ERes" ? enterpriseResourceMap.get(item.toLowerCase()) : sourceText === "PRes" ? projectResourceMap.get(item.toLowerCase()) : null;
          const created = await createCostToCompleteDetail(project.id, {
            cost_code_id: costCode.id,
            resource_source: sourceText === "ERes" || sourceText === "PRes" ? sourceText : null,
            item: resource?.resource_id ?? item || null,
            description: resource?.resource_name ?? (row.Description ?? "").trim() || null,
            unit: resource?.unit ?? (row.Unit ?? "").trim() || null,
            rate: resource ? Number(resource.rate) : parseNumber(row.Rate),
            category: resource?.category ?? ((row.Category ?? "").trim() ? row.Category as ResourceCategory : null),
  ...
  async function applyBulkEdit() {
    ...
    if (mode === "ctc" && ["description", "unit", "rate"].includes(bulkField) && selected.some((row) => (row as CtcGridRow).resource_source)) {
      setError("Description, Unit and Rate are locked for rows added from Enterprise or Project Resource Rates. Change the resource through the resource picker instead.");
      return;
    }
    setSaving(true); setError("");
    try {
      for (const selectedRow of selected) {
        if (mode === "actual") {
          ...
          else if (bulkField === "amount") await updateActualCostTransaction(row.id, { amount: Number(bulkValue) });
          ...
          if (bulkField.startsWith("period:")) await setCostToCompletePeriodQty(row.id, bulkField.slice(7), Number(bulkValue));
          else if (bulkField === "rate") await updateCostToCompleteDetail(row.id, { rate: Number(bulkValue) });
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 12000, background: "#f5f7fa", display: "flex", flexDirection: "column" }}>
    ...
    <div className="enterprise-toolbar" style={{ flexWrap: "wrap", padding: "8px 12px", background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
      ...
      <div style={{ display: "inline-flex", alignItems: "center", border: "1px solid #d0d5dd", borderRadius: 6, overflow: "hidden", background: "#fff" }}>
        <button className="button primary" style={{ borderRadius: 0, border: 0 }} disabled={loading || saving || (mode === "actual" && periods.length === 0)} onClick={() => void addRows()}>+ Add Row</button>
        <input aria-label="Number of rows to add" title="Rows to add (1–100)" type="number" min={1} max={100} value={addCount} onChange={(event) => setAddCount(Math.max(1, Math.min(100, Number(event.target.value) || 1)))} style={{ width: 54, height: 30, border: 0, borderLeft: "1px solid #d0d5dd", textAlign: "center", fontSize: 12 }}/>
      </div>
      {mode === "ctc" && <button className="button secondary" title="Add from Resource Rates" onClick={() => setResourcePaneOpen(true)} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><SvgIcon type="resource"/> Resources</button>}
      ...
      {loading ? <div className="data-message" style={{ height: "100%" }}><span className="spinner"/>Loading {title} records…</div> : <AgGridProvider modules={[AllEnterpriseModule]} licenseKey={process.env.NEXT_PUBLIC_AG_GRID_LICENSE_KEY ?? ""}>
        <div className="enterprise-grid-card" style={{ height: "100%", width: "100%", overflow: "hidden" }}>
          <AgGridReact<RelatedGridRow>
            ...
            onCellFocused={(event) => { if (event.rowIndex != null) setFocusedRowId(event.api.getDisplayedRowAtIndex(event.rowIndex)?.data?.id ?? null); }}
    ...
    {resourcePaneOpen && mode === "ctc" && <div style={{ position: "fixed", inset: 0, zIndex: 13000, pointerEvents: "none" }}>
      <button aria-label="Close resource picker" onClick={() => setResourcePaneOpen(false)} style={{ position: "absolute", inset: 0, border: 0, background: "rgba(15,23,42,.12)", pointerEvents: "auto" }}/>
      <aside style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "min(430px, 92vw)", background: "#fff", borderLeft: "1px solid #dfe4ea", boxShadow: "-12px 0 32px rgba(15,23,42,.12)", display: "flex", flexDirection: "column", pointerEvents: "auto" }}>
        <div style={{ padding: 14, borderBottom: "1px solid #e5e7eb", display: "flex", alignItems: "center", gap: 10 }}><div><strong>Add Resource</strong><div style={{ fontSize: 11, color: "#667085" }}>Adds below the selected row. Item, Description, Unit and Rate will be locked.</div></div><button className="button secondary compact" style={{ marginLeft: "auto" }} onClick={() => setResourcePaneOpen(false)}>✕</button></div>
        <div style={{ padding: 12, display: "grid", gap: 10, borderBottom: "1px solid #e5e7eb" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}><button className={resourceScope === "enterprise" ? "button primary" : "button secondary"} onClick={() => setResourceScope("enterprise")}>Enterprise</button><button className={resourceScope === "project" ? "button primary" : "button secondary"} onClick={() => setResourceScope("project")}>Project</button></div>
          <input value={resourceSearch} onChange={(event) => setResourceSearch(event.target.value)} placeholder="Search Resource ID, name or category…"/>
        </div>
        <div style={{ flex: 1, minHeight: 0, overflow: "auto", padding: 8 }}>
          {pickableResources.map((resource) => <div key={`${resource.scope}:${resource.resource_id}`} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "center", padding: "9px 8px", borderBottom: "1px solid #eef1f4" }}>
            <div style={{ minWidth: 0 }}><div style={{ display: "flex", gap: 7, alignItems: "baseline" }}><strong style={{ fontSize: 12 }}>{resource.resource_id}</strong><span style={{ fontSize: 11, color: "#667085", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{resource.resource_name}</span></div><div style={{ marginTop: 3, fontSize: 10.5, color: "#7b8491" }}>{resource.category} · {resource.unit} · {numberFormat(resource.rate, 4)}</div></div>
            <button className="button secondary compact" disabled={saving} title={`Add ${resource.resource_id}`} onClick={() => void addResource(resource)} style={{ width: 30, paddingInline: 0, display: "grid", placeItems: "center" }}><SvgIcon type="add"/></button>
          </div>)}
          {!pickableResources.length && <div className="data-message">No active resources match your search.</div>}
        </div>
      </aside>
    </div>}
