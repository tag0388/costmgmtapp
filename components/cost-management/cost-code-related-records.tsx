...
export function CostCodeActionsCell({ costCode, project, onEdit }: { costCode: CostCode; project: Project | null; onEdit: () => void }) {
  ...
  const menu = typeof document !== "undefined" && open ? createPortal(
    <div ref={menuRef} onMouseDown={(event) => event.stopPropagation()} style={{ position: "fixed", right: menuPosition.right, top: menuPosition.top, zIndex: 10000, width: 188, background: "#fff", border: "1px solid #dbe1e8", borderRadius: 6, boxShadow: "0 10px 26px rgba(15,23,42,.16)", padding: 4 }}>
...
function MenuOption({ icon, label, hint, disabled, onClick }: { icon: "changes" | "actual" | "ctc"; label: string; hint: string; disabled?: boolean; onClick: () => void }) {
  return <button disabled={disabled} onClick={onClick} style={{ width: "100%", border: 0, background: "transparent", borderRadius: 4, padding: "6px 7px", display: "grid", gridTemplateColumns: "20px 1fr auto", alignItems: "center", gap: 7, textAlign: "left", cursor: disabled ? "not-allowed" : "pointer", color: disabled ? "#9ca3af" : "#111827", font: "inherit" }}>
    <span style={{ display: "grid", placeItems: "center", color: disabled ? "#9ca3af" : "#4b5563" }}><SvgIcon type={icon}/></span>
    <span style={{ fontSize: 12, fontWeight: 600 }}>{label}</span>
    <small style={{ fontSize: 10, color: disabled ? "#b5bbc4" : "#7b8491", whiteSpace: "nowrap" }}>{hint}</small>
  </button>;
}
...
function RelatedRecordsWorkspace({ project, costCode, mode, onClose }: { project: Project; costCode: CostCode; mode: RelatedMode; onClose: () => void }) {
  ...
  async function ctcChanged(event: CellValueChangedEvent<CtcGridRow>) {
    ...
    try {
      ...
      else if (colId === "resource_source_label") { const source = event.newValue === "ERes" || event.newValue === "PRes" ? event.newValue as CtcResourceSource : null; event.data.resource_source = source; const validIds = source === "ERes" ? enterpriseResourceIds : source === "PRes" ? projectResourceIds : null; const currentItem = event.data.item?.trim() ?? ""; const clearItem = !!(validIds && currentItem && !validIds.has(currentItem.toLowerCase())); if (clearItem) { event.data.item = null; event.api.refreshCells({ rowNodes: [event.node], columns: ["item"], force: true }); } await updateCostToCompleteDetail(event.data.id, { resource_source: source, ...(clearItem ? { item: null } : {}) }); }
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 12000, background: "#f5f7fa", display: "flex", flexDirection: "column" }}>
    ...
    {bulkOpen && <div className="confirm-layer"><button className="confirm-scrim" onClick={() => !saving && setBulkOpen(false)} aria-label="Close bulk edit"/><div className="confirm-dialog" role="dialog" aria-modal="true" style={{ width: "min(520px, 92vw)" }}><h2>Bulk Edit {selectedCount} Row{selectedCount === 1 ? "" : "s"}</h2><p>Choose one field and apply the same value to all selected rows.</p><div style={{ display: "grid", gap: 10, textAlign: "left" }}><label><span>Field</span><select value={bulkField} onChange={(event) => { setBulkField(event.target.value); setBulkValue(""); }}><option value="">Select field…</option>{bulkChoices.map((choice) => <option key={choice.id} value={choice.id}>{choice.label}</option>)}</select></label>{chosenBulk && <label><span>Value</span>{chosenBulk.values ? <select value={bulkValue} onChange={(event) => setBulkValue(event.target.value)}>{chosenBulk.values.map((value) => <option key={value || "blank"} value={value}>{value || "(Blank)"}</option>)}</select> : <input type={chosenBulk.kind === "number" || chosenBulk.kind === "periodQty" ? "number" : "text"} value={bulkValue} onChange={(event) => setBulkValue(event.target.value)}/>}</label>}</div><div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}><button className="button secondary" onClick={() => setBulkOpen(false)} disabled={saving}>Cancel</button><button className="button primary" onClick={() => void applyBulkEdit()} disabled={!bulkField || saving}>Apply</button></div></div></div>}

    {showSaveView && <div className="confirm-layer"><button className="confirm-scrim" onClick={() => setShowSaveView(false)} aria-label="Close save view"/><div className="confirm-dialog" role="dialog" aria-modal="true" style={{ width: "min(440px, 92vw)" }}><h2>Save Grid View</h2><p>Save the current column order, grouping and filters for this {title} workspace.</p><label style={{ textAlign: "left" }}><span>View Name</span><input value={viewName} onChange={(event) => setViewName(event.target.value)} autoFocus/></label><div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 18 }}><button className="button secondary" onClick={() => setShowSaveView(false)}>Cancel</button><button className="button primary" disabled={!viewName.trim()} onClick={() => void saveView()}>Save</button></div></div></div>}
