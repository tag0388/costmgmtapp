...
type BulkChoice = {
  id: string;
  label: string;
  kind: "text" | "number" | "select" | "attribute" | "periodQty";
  values?: string[];
  definition?: AttributeDefinition;
};
...
function SvgIcon({ type }: { type: "changes" | "actual" | "ctc" | "more" | "resources" }) {
  ...
  if (type === "resources") return <svg {...common}><path d="M4 5h16v4H4z"/><path d="M4 11h16v8H4z"/><path d="M8 5v4"/><path d="M8 11v8"/><path d="M14 11v8"/></svg>;
...
function formatPeriodDate(value: string) {
  ...
  return Number.isNaN(date.getTime())
    ? value
    : new Intl.DateTimeFormat("en-AU", { day: "2-digit", month: "2-digit", year: "2-digit" }).format(date);
}
function periodLabel(period: CostReportingPeriod) {
  return `P${period.period_number} | ${formatPeriodDate(period.end_date)}`;
}
function periodHeader(period: CostReportingPeriod) {
  return `P${period.period_number}\n${formatPeriodDate(period.end_date)}`;
}
function periodExcel(period: CostReportingPeriod) {
  return `P${period.period_number} | ${formatPeriodDate(period.end_date)}`;
}
function numberFormat(value: unknown, decimals = 2) {
  const number = Number(value ?? 0);
  return Number.isFinite(number)
    ? new Intl.NumberFormat(undefined, { minimumFractionDigits: 0, maximumFractionDigits: decimals }).format(number)
    : "";
}
...
function buildAttributes<TField extends string>(
  ...
  return active.map((attribute) => {
    ...
    return {
      ...attribute,
      columnName: (counts.get(name.toLowerCase()) ?? 0) > 1
        ? `${name} (${attribute.prefix}${String(attribute.definition.attribute_number).padStart(2, "0")})`
        : name,
    };
  });
}
...
function insertAfterAnchor<T extends { id: string }>(current: T[], added: T[], anchorId: string | null) {
...
function RelatedRecordsWorkspace({ project, costCode, mode, onClose }: { project: Project; costCode: CostCode; mode: RelatedMode; onClose: () => void }) {
  ...
  const actualGridRows = useMemo<ActualGridRow[]>(() => actualRows.map((row) => ({
    ...row,
    period_label: periodById.get(row.cost_period_id) ? periodLabel(periodById.get(row.cost_period_id)!) : "",
  })), [actualRows, periodById]);
  ...
  const visibleResources = useMemo(() => {
    ...
    if (!q) return source;
    return source.filter((resource) => [resource.resource_id, resource.resource_name, resource.unit, resource.category].some((value) => String(value ?? "").toLowerCase().includes(q)));
  ...
  function syncGroupState(api: GridApi<RelatedGridRow> | null = gridApi) {
    setHasGroups((api?.getRowGroupColumns().length ?? 0) > 0);
  }
  function selectionChanged(event: SelectionChangedEvent<RelatedGridRow>) {
    const selected = event.api.getSelectedRows();
    setSelectedCount(selected.length);
    if (selected.length) setAnchorRowId(selected[selected.length - 1].id);
  }
  function cellClicked(event: CellClickedEvent<RelatedGridRow>) {
    if (event.data?.id) setAnchorRowId(event.data.id);
  }
  ...
  const actualColumnDefs = useMemo<ColDef<ActualGridRow>[]>(() => [
    ...
    { field: "period_label", colId: "period_label", headerName: "Cost Reporting Period", editable: true, cellEditor: "agSelectCellEditor", cellEditorParams: { values: periods.map(periodLabel) }, filter: "agSetColumnFilter", minWidth: 130 },
  ...
  const ctcColumnDefs = useMemo<ColDef<CtcGridRow>[]>(() => [
    { field: "resource_source_label", colId: "resource_source_label", headerName: "Resource Source", editable: false, filter: "agSetColumnFilter", minWidth: 100, maxWidth: 120 },
    { field: "item", headerName: "Item", editable: (params) => !params.data?.resource_source, filter: true, minWidth: 105 },
    { field: "description", headerName: "Description", editable: (params) => !params.data?.resource_source, filter: true, minWidth: 180 },
    { colId: "qty", headerName: "Qty", editable: false, type: "numericColumn", aggFunc: "sum", enableValue: true, valueGetter: (params) => Object.values(params.data?.period_qty ?? {}).reduce((sum, value) => sum + Number(value || 0), 0), valueFormatter: (params) => numberFormat(params.value, 4), minWidth: 75, maxWidth: 95 },
    { field: "unit", headerName: "Unit", editable: (params) => !params.data?.resource_source, filter: "agSetColumnFilter", minWidth: 70, maxWidth: 90 },
    { field: "rate", headerName: "Rate", editable: (params) => !params.data?.resource_source, type: "numericColumn", valueParser: (params) => Number(params.newValue), valueFormatter: (params) => numberFormat(params.value, 4), minWidth: 80, maxWidth: 110 },
    { colId: "total", headerName: "Total", editable: false, type: "numericColumn", aggFunc: "sum", enableValue: true, valueGetter: (params) => Object.values(params.data?.period_qty ?? {}).reduce((sum, value) => sum + Number(value || 0), 0) * Number(params.data?.rate ?? 0), valueFormatter: (params) => numberFormat(params.value, 2), minWidth: 90, maxWidth: 120 },
    { field: "category", headerName: "Category", editable: true, cellEditor: "agSelectCellEditor", cellEditorParams: { values: ["", ...RESOURCE_CATEGORIES] }, filter: "agSetColumnFilter", valueFormatter: (params) => params.value ?? "", minWidth: 90, maxWidth: 115 },
  ...
  async function ctcChanged(event: CellValueChangedEvent<CtcGridRow>) {
    ...
    try {
      ...
      if (colId.startsWith("period:")) {
        ...
        if (event.data.resource_source) throw new Error("Resource Item is read-only. Remove the resource row and add a different resource instead.");
        await updateCostToCompleteDetail(event.data.id, { item: event.newValue ? String(event.newValue).trim() : null });
      } else if (colId === "rate") {
        if (event.data.resource_source) throw new Error("Resource Rate is read-only.");
        ...
        if (event.data.resource_source) throw new Error("Resource Description is read-only.");
        await updateCostToCompleteDetail(event.data.id, { description: event.newValue ? String(event.newValue) : null });
      } else if (colId === "unit") {
        if (event.data.resource_source) throw new Error("Resource Unit is read-only.");
  ...
  async function addRows() {
    const count = Math.max(1, Math.min(100, Math.floor(Number(addCount) || 1)));
    setAddCount(count);
    setSaving(true);
    setError("");
    try {
      if (mode === "actual") {
        ...
        for (let index = 0; index < count; index += 1) {
          created.push(await createActualCostTransaction(project.id, {
        ...
        setActualRows((current) => insertAfterAnchor(current, created, anchorRowId));
        setAnchorRowId(created[created.length - 1]?.id ?? anchorRowId);
      } else {
        ...
        setCtcRows((current) => insertAfterAnchor(current, created, anchorRowId));
        setAnchorRowId(created[created.length - 1]?.id ?? anchorRowId);
      }
      showNotice(`${count} row${count === 1 ? "" : "s"} added${anchorRowId ? " below the selected row" : ""}.`);
    } catch (requestError) {
      setError(mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError));
    } finally {
      setSaving(false);
  ...
  async function addResource(resource: PickerResource) {
    const source: CtcResourceSource = resourceScope === "enterprise" ? "ERes" : "PRes";
    setSaving(true);
    setError("");
    try {
      ...
      setCtcRows((current) => insertAfterAnchor(current, [created], anchorRowId));
      setAnchorRowId(created.id);
      showNotice(`${resource.resource_id} added from ${resourceScope === "enterprise" ? "Enterprise" : "Project"} Resources.`);
    } catch (requestError) {
      setError(costToCompleteErrorMessage(requestError));
    } finally {
      setSaving(false);
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      gridApi?.deselectAll();
      setSelectedCount(0);
      if (anchorRowId && ids.includes(anchorRowId)) setAnchorRowId(null);
  ...
  const excelColumns = useMemo(() => mode === "actual" ? [
    ...
    "Resource Source", "Item", "Description", "Unit", "Rate", "Category", ...ctcAttributes.map((attribute) => attribute.columnName), ...periods.map(periodExcel),
  ...
  function exportRows() {
    const data: ExcelRow[] = mode === "actual" ? actualGridRows.map((row) => ({
      ...
      "Cost Reporting Period": periodById.get(row.cost_period_id) ? periodExcel(periodById.get(row.cost_period_id)!) : "",
    ...
    exportExcel(
      `${project.project_code}-${costCode.cost_code_id}-${mode === "actual" ? "actual-cost" : "cost-to-complete"}`,
      mode === "actual" ? "Actual Cost" : "Cost to Complete",
      data.length ? data : [template],
    );
  }
    ...
    try {
      ...
      incoming.forEach((row, index) => {
        ...
        if (mode === "actual") {
          ...
          actualAttributes.forEach((attribute) => {
            const value = (row[attribute.columnName] ?? "").trim();
            if (value && !attribute.definition.attribute_values.some((option) => option.is_active && option.value_id.toLowerCase() === value.toLowerCase())) errors.push(`Row ${line}: ${attribute.columnName} must contain an active Value ID.`);
          });
        } else {
          ...
          if (!sourceText) {
            ...
            if (category && !RESOURCE_CATEGORIES.includes(category as ResourceCategory)) errors.push(`Row ${line}: Category is invalid.`);
          }
          ctcAttributes.forEach((attribute) => {
            const value = (row[attribute.columnName] ?? "").trim();
            if (value && !attribute.definition.attribute_values.some((option) => option.is_active && option.value_id.toLowerCase() === value.toLowerCase())) errors.push(`Row ${line}: ${attribute.columnName} must contain an active Value ID.`);
          });
          periods.forEach((period) => {
            const qty = parseNumber(row[periodExcel(period)]);
            if (Number.isNaN(qty) || qty < 0) errors.push(`Row ${line}: ${periodExcel(period)} must be zero or greater.`);
          });
        }
      });
    ...
    try {
      ...
      for (let index = 0; index < importRows.length; index += 1) {
        ...
        if (mode === "actual") {
          ...
          const sourceText = (row["Resource Source"] ?? "").trim();
          const item = (row.Item ?? "").trim();
          const resource = sourceText === "ERes"
            ? enterpriseResourceById.get(item.toLowerCase())
            : sourceText === "PRes"
              ? projectResourceById.get(item.toLowerCase())
              : null;
          const created = await createCostToCompleteDetail(project.id, {
            cost_code_id: costCode.id,
            resource_source: sourceText === "ERes" || sourceText === "PRes" ? sourceText : null,
            item: resource ? resource.resource_id : item || null,
            description: resource ? resource.resource_name : (row.Description ?? "").trim() || null,
            unit: resource ? resource.unit : (row.Unit ?? "").trim() || null,
            rate: resource ? resource.rate : parseNumber(row.Rate),
          ...
          for (const period of periods) {
            const qty = parseNumber(row[periodExcel(period)]);
            if (qty !== 0) await setCostToCompletePeriodQty(created.id, period.id, qty);
          }
        }
        setProgress(((index + 1) / Math.max(importRows.length, 1)) * 100);
      }
      setImportRows(null);
      showNotice(`${mode === "actual" ? "Actual Cost" : "Cost to Complete"} imported for ${costCode.cost_code_id}.`);
      await refresh();
    } catch (requestError) {
      setImportErrors([mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError)]);
    } finally {
      setImporting(false);
    }
  }
  ...
  async function applyBulkEdit() {
    ...
    try {
      if (mode === "ctc" && ["description", "unit", "rate"].includes(bulkField) && selected.some((row) => (row as CtcGridRow).resource_source)) {
        throw new Error("Item, Description, Unit and Rate are locked for Enterprise/Project resource rows. Bulk edit those fields only on user-added rows.");
      }
      for (const selectedRow of selected) {
        if (mode === "actual") {
          const row = selectedRow as ActualGridRow;
          if (bulkField === "period_label") {
            const period = periodByLabel.get(bulkValue);
            if (!period) throw new Error("Select a valid Cost Reporting Period.");
            await updateActualCostTransaction(row.id, { cost_period_id: period.id, transaction_date: period.end_date });
          } else if (bulkField === "amount") {
            const value = Number(bulkValue);
            if (!Number.isFinite(value)) throw new Error("Amount must be a valid number.");
            await updateActualCostTransaction(row.id, { amount: value });
          } else if (bulkField === "transaction_id") {
            await updateActualCostTransaction(row.id, { transaction_id: bulkValue || null });
          } else if (bulkField === "description") {
            await updateActualCostTransaction(row.id, { description: bulkValue });
          } else if (bulkField === "transaction_type") {
            await updateActualCostTransaction(row.id, { transaction_type: bulkValue as TransactionType });
          } else {
            await updateActualCostTransaction(row.id, { [bulkField]: bulkValue || null } as Partial<Record<ActualAttributeField, string | null>>);
          }
        } else {
          const row = selectedRow as CtcGridRow;
          if (bulkField.startsWith("period:")) {
            const qty = Number(bulkValue);
            if (!Number.isFinite(qty) || qty < 0) throw new Error("Period quantity must be zero or greater.");
            await setCostToCompletePeriodQty(row.id, bulkField.slice(7), qty);
          } else if (bulkField === "rate") {
            const rate = Number(bulkValue);
            if (!Number.isFinite(rate) || rate < 0) throw new Error("Rate must be zero or greater.");
            await updateCostToCompleteDetail(row.id, { rate });
          } else if (bulkField === "description") {
            await updateCostToCompleteDetail(row.id, { description: bulkValue || null });
          } else if (bulkField === "unit") {
            await updateCostToCompleteDetail(row.id, { unit: bulkValue || null });
          } else if (bulkField === "category") {
            await updateCostToCompleteDetail(row.id, { category: bulkValue ? bulkValue as ResourceCategory : null });
          } else {
            await updateCostToCompleteDetail(row.id, { [bulkField]: bulkValue || null } as Partial<Record<CtcAttributeField, string | null>>);
          }
        }
      }
      setBulkOpen(false);
      setBulkField("");
      setBulkValue("");
      showNotice(`Updated ${selected.length} selected row${selected.length === 1 ? "" : "s"}.`);
      await refresh();
    } catch (requestError) {
      setError(mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError));
    } finally {
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 12000, background: "#f5f7fa", display: "flex", flexDirection: "column" }}>
    ...
    <div className="enterprise-toolbar" style={{ flexWrap: "wrap", padding: "8px 12px", background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
      ...
      <div style={{ display: "inline-flex", alignItems: "stretch", borderRadius: 6, overflow: "hidden", boxShadow: "0 0 0 1px rgba(37,99,235,.25)" }}>
        <button className="button primary" style={{ borderRadius: 0 }} disabled={loading || saving || (mode === "actual" && periods.length === 0)} onClick={() => void addRows()}>+ Add Row</button>
        <input
          aria-label="Number of rows to add"
          title="Rows to add (maximum 100)"
          type="number"
          min={1}
          max={100}
          value={addCount}
          onChange={(event) => setAddCount(Math.max(1, Math.min(100, Number(event.target.value) || 1)))}
          style={{ width: 56, border: 0, borderLeft: "1px solid rgba(255,255,255,.35)", padding: "0 7px", textAlign: "center", fontSize: 12 }}
        />
      </div>
      {mode === "ctc" && <button className="button secondary" title="Add Enterprise or Project Resource" onClick={() => setResourcePaneOpen((value) => !value)} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><SvgIcon type="resources"/> Resources</button>}
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", overflow: "hidden" }}>
      ...
      {mode === "ctc" && resourcePaneOpen && <aside style={{ width: 360, maxWidth: "38vw", minWidth: 300, background: "#fff", borderLeft: "1px solid #dfe4ea", display: "flex", flexDirection: "column", minHeight: 0 }}>
        <div style={{ padding: "10px 12px", borderBottom: "1px solid #e5e7eb", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ display: "grid", placeItems: "center" }}><SvgIcon type="resources"/></span>
          <div><div style={{ fontSize: 13, fontWeight: 700 }}>Add Resource</div><div style={{ fontSize: 10.5, color: "#6b7280" }}>Creates a locked resource snapshot</div></div>
        ...
        <div style={{ padding: 10, borderBottom: "1px solid #e5e7eb", display: "grid", gap: 8 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, background: "#f3f4f6", padding: 3, borderRadius: 6 }}>
            <button className={resourceScope === "enterprise" ? "button primary compact" : "button secondary compact"} onClick={() => setResourceScope("enterprise")}>Enterprise</button>
            <button className={resourceScope === "project" ? "button primary compact" : "button secondary compact"} onClick={() => setResourceScope("project")}>Project</button>
          </div>
          <label className="enterprise-search" style={{ width: "100%" }}><span>⌕</span><input value={resourceSearch} onChange={(event) => setResourceSearch(event.target.value)} placeholder="Search resources…"/></label>
        </div>
        <div style={{ flex: 1, minHeight: 0, overflow: "auto", padding: 8 }}>
          {visibleResources.length === 0 ? <div style={{ padding: 16, textAlign: "center", color: "#6b7280", fontSize: 12 }}>No active resources found.</div> : visibleResources.map((resource) => <div key={`${resourceScope}-${resource.id}`} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center", padding: "8px 7px", borderBottom: "1px solid #eef0f3" }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700 }}>{resource.resource_id}</div>
              <div style={{ fontSize: 11, color: "#4b5563", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{resource.resource_name}</div>
              <div style={{ fontSize: 10, color: "#7b8491", marginTop: 2 }}>{resource.category} · {resource.unit} · {numberFormat(resource.rate, 4)}</div>
            </div>
            <button className="button secondary compact" disabled={saving} onClick={() => void addResource(resource)}>+ Add</button>
          </div>)}
        </div>
      </aside>}
    </div>
    ...
    <footer className="grid-footer" style={{ padding: "6px 14px", background: "#fff", borderTop: "1px solid #e5e7eb" }}>
      <span>{rows.length} related {title} row{rows.length === 1 ? "" : "s"} · {selectedCount} selected</span>
      <span>{mode === "ctc" ? `${periods.length} phasing periods` : `${periods.length} reporting periods`} · Cost Code is fixed to {costCode.cost_code_id}</span>
    </footer>
