...
type ResourceOption = { key: string; source: "ERes" | "PRes"; resource_id: string; resource_name: string; category: ResourceCategory; rate: number; unit: string };
...
function validateHeaders(rows: ExcelRow[], expected: string[]) {
  ...
  return actual.length !== expected.length || expected.some((column, index) => actual[index] !== column) ? [`Columns must be exactly: ${expected.join(", ")}.`] : [];
}
...
function lockedStyle(params: any) {
...
export function CostCodeActionsCell({ costCode, project, onEdit }: { costCode: CostCode; project: Project | null; onEdit: () => void }) {
  ...
  const workspace = typeof document !== "undefined" && project && mode ? createPortal(<RelatedRecordsWorkspace project={project} costCode={costCode} mode={mode} onClose={() => setMode(null)}/>, document.body) : null;
...
function MenuOption({ icon, label, hint, disabled, onClick }: { icon: "changes" | "actual" | "ctc"; label: string; hint: string; disabled?: boolean; onClick: () => void }) {
  return <button disabled={disabled} onClick={onClick} style={{ width: "100%", border: 0, background: "transparent", borderRadius: 4, padding: "5px 7px", display: "grid", gridTemplateColumns: "20px 1fr auto", alignItems: "center", gap: 6, textAlign: "left", cursor: disabled ? "not-allowed" : "pointer", color: disabled ? "#9ca3af" : "#111827", font: "inherit" }}>
    <span style={{ display: "grid", placeItems: "center", color: disabled ? "#9ca3af" : "#4b5563" }}><SvgIcon type={icon}/></span><span style={{ fontSize: 11.5, fontWeight: 600 }}>{label}</span><small style={{ fontSize: 9.5, color: disabled ? "#b5bbc4" : "#7b8491" }}>{hint}</small>
  </button>;
}
...
function RelatedRecordsWorkspace({ project, costCode, mode, onClose }: { project: Project; costCode: CostCode; mode: RelatedMode; onClose: () => void }) {
  ...
  const resourceOptions = useMemo<ResourceOption[]>(() => {
    ...
    const mapped: ResourceOption[] = source.map((row) => ({ key: `${resourceScope}:${row.id}`, source: resourceScope === "enterprise" ? "ERes" : "PRes", resource_id: row.resource_id, resource_name: row.resource_name, category: row.category, rate: Number(row.rate), unit: row.unit }));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [reportingPeriods, enterpriseDefs, projectDefs, savedViews] = await Promise.all([
        listCostReportingPeriods(project.id), listEnterpriseAttributes(project.enterprise_id, "Line Item"), listProjectAttributes(project.id, "Line Item"), listProjectGridViews(project.id, gridKey),
      ]);
      ...
      if (mode === "actual") {
        ...
        const [ledgerRows, eResources, pResources] = await Promise.all([listCostToCompleteLedgerForCostCode(project.id, costCode.id), listResourceRates(project.enterprise_id), listProjectResourceRates(project.id)]);
        setCtcRows(sortByOrder(ledgerRows)); setActualRows([]); setEnterpriseResources(eResources); setProjectResources(pResources);
      }
      setSelectedCount(0);
    } catch (requestError) { setError(mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError)); }
    finally { setLoading(false); }
  }, [costCode.id, gridKey, mode, project.enterprise_id, project.id]);
  ...
  async function actualChanged(event: CellValueChangedEvent<ActualGridRow>) {
    ...
    try {
      ...
      if (colId === "period_label") { const period = periodByLabel.get(String(event.newValue).trim().toLowerCase()); if (!period) throw new Error("Select a valid Cost Reporting Period."); await updateActualCostTransaction(event.data.id, { cost_period_id: period.id, transaction_date: period.end_date }); }
  ...
  async function ctcChanged(event: CellValueChangedEvent<CtcGridRow>) {
    ...
    try {
      const colId = event.column.getColId();
      if (colId.startsWith("period:")) { const qty = Number(event.newValue); if (!Number.isFinite(qty) || qty < 0) throw new Error("Period quantity must be zero or greater."); await setCostToCompletePeriodQty(event.data.id, colId.slice(7), qty); }
  ...
  function focusedRowId() {
    ...
    const ranked = gridApi.getSelectedRows().map((row) => ({ row, index: gridApi.getRowNode(row.id)?.rowIndex ?? -1 })).sort((a, b) => b.index - a.index);
    return ranked[0]?.row.id ?? null;
  }
  function insertionOrders(count: number) {
    const base: Array<{ id: string; row_order: number | null; created_at: string }> = mode === "actual" ? sortByOrder(actualRows) : sortByOrder(ctcRows);
    const anchorId = focusedRowId();
    if (!base.length) return Array.from({ length: count }, (_, index) => (index + 1) * 1000);
    if (!anchorId) { const last = base[base.length - 1].row_order ?? base.length * 1000; return Array.from({ length: count }, (_, index) => Number((last + (index + 1) * 1000).toFixed(6))); }
    const index = base.findIndex((row) => row.id === anchorId);
    if (index < 0) { const last = base[base.length - 1].row_order ?? base.length * 1000; return Array.from({ length: count }, (_, offset) => Number((last + (offset + 1) * 1000).toFixed(6))); }
  ...
  async function addRows() {
    ...
    const orders = insertionOrders(count); const hadAnchor = Boolean(focusedRowId());
    setSaving(true); setError("");
    try {
      if (mode === "actual") {
        ...
        for (let index = 0; index < count; index += 1) created.push(await createActualCostTransaction(project.id, { cost_period_id: period.id, cost_code_id: costCode.id, transaction_date: period.end_date, transaction_id: null, description: "", amount: 0, transaction_type: "MAN", row_order: orders[index], ...Object.fromEntries(actualAttributes.map((attribute) => [attribute.field, null])) }));
        setActualRows((current) => sortByOrder([...current, ...created]));
      } else {
        const created: CostToCompleteLedgerRow[] = [];
        for (let index = 0; index < count; index += 1) created.push(await createCostToCompleteDetail(project.id, { cost_code_id: costCode.id, item: null, description: null, unit: null, rate: 0, category: null, resource_source: null, row_order: orders[index], ...Object.fromEntries(ctcAttributes.map((attribute) => [attribute.field, null])) }));
        setCtcRows((current) => sortByOrder([...current, ...created]));
      }
      showNotice(`Inserted ${count} row${count === 1 ? "" : "s"}${hadAnchor ? " below the selected row" : ""}.`);
    } catch (requestError) { setError(mode === "actual" ? actualCostErrorMessage(requestError) : costToCompleteErrorMessage(requestError)); }
    finally { setSaving(false); }
  }
  ...
  async function addSelectedResources() {
    ...
    try {
      ...
      for (let index = 0; index < selected.length; index += 1) {
        ...
        created.push(await createCostToCompleteDetail(project.id, { cost_code_id: costCode.id, resource_source: resource.source, item: resource.resource_id, description: resource.resource_name, unit: resource.unit, rate: resource.rate, category: resource.category, row_order: orders[index], ...Object.fromEntries(ctcAttributes.map((attribute) => [attribute.field, null])) }));
      }
      setCtcRows((current) => sortByOrder([...current, ...created])); setSelectedResources(new Set()); showNotice(`Added ${selected.length} resource${selected.length === 1 ? "" : "s"}. Resource fields are locked.`);
  ...
  const excelColumns = useMemo(() => mode === "actual" ? ["Item", "Description", "Transaction Type", "Amount", "Cost Reporting Period", ...actualAttributes.map((attribute) => attribute.columnName)] : ["Resource Source", "Item", "Description", "Unit", "Rate", "Category", ...ctcAttributes.map((attribute) => attribute.columnName), ...periods.map(periodLabel)], [actualAttributes, ctcAttributes, mode, periods]);
  function exportRows() {
    const data: ExcelRow[] = mode === "actual" ? actualGridRows.map((row) => ({ Item: row.transaction_id ?? "", Description: row.description, "Transaction Type": row.transaction_type, Amount: String(row.amount), "Cost Reporting Period": periodById.get(row.cost_period_id) ? periodLabel(periodById.get(row.cost_period_id)!) : "", ...Object.fromEntries(actualAttributes.map((attribute) => [attribute.columnName, row[attribute.field] ?? ""])) })) : ctcGridRows.map((row) => ({ "Resource Source": row.resource_source ?? "", Item: row.item ?? "", Description: row.description ?? "", Unit: row.unit ?? "", Rate: String(row.rate), Category: row.category ?? "", ...Object.fromEntries(ctcAttributes.map((attribute) => [attribute.columnName, row[attribute.field] ?? ""])), ...Object.fromEntries(periods.map((period) => [periodLabel(period), String(row.period_qty[period.id] ?? 0)])) }));
  ...
  async function chooseImport(file: File | undefined) {
    if (!file) return;
    try {
      const incoming = await readExcel(file); const errors = validateHeaders(incoming, excelColumns);
  ...
  async function runImport() {
    ...
    try {
      if (replace) { if (mode === "actual") await deleteActualCostTransactions(actualRows.map((row) => row.id)); else await deleteCostToCompleteDetails(ctcRows.map((row) => row.id)); }
      const startOrder = replace ? 0 : rows.reduce((max, row) => Math.max(max, row.row_order ?? 0), 0);
      for (let index = 0; index < importRows.length; index += 1) {
        const row = importRows[index]; const rowOrder = startOrder + (index + 1) * 1000;
        if (mode === "actual") {
          const period = periodByLabel.get((row["Cost Reporting Period"] ?? "").trim().toLowerCase())!;
          await createActualCostTransaction(project.id, { cost_code_id: costCode.id, cost_period_id: period.id, transaction_date: period.end_date, transaction_id: (row.Item ?? "").trim() || null, description: (row.Description ?? "").trim(), transaction_type: row["Transaction Type"] as TransactionType, amount: parseNumber(row.Amount), row_order: rowOrder, ...Object.fromEntries(actualAttributes.map((attribute) => [attribute.field, (row[attribute.columnName] ?? "").trim() || null])) });
        } else {
          const sourceText = (row["Resource Source"] ?? "").trim();
          const created = await createCostToCompleteDetail(project.id, { cost_code_id: costCode.id, resource_source: sourceText === "ERes" || sourceText === "PRes" ? sourceText : null, item: (row.Item ?? "").trim() || null, description: (row.Description ?? "").trim() || null, unit: (row.Unit ?? "").trim() || null, rate: parseNumber(row.Rate), category: (row.Category ?? "").trim() ? row.Category as ResourceCategory : null, row_order: rowOrder, ...Object.fromEntries(ctcAttributes.map((attribute) => [attribute.field, (row[attribute.columnName] ?? "").trim() || null])) });
  ...
  const bulkChoices = useMemo<BulkChoice[]>(() => mode === "actual" ? [
    ...
    { id: "item", label: "Item", kind: "text", userOnly: true }, { id: "description", label: "Description", kind: "text", userOnly: true }, { id: "unit", label: "Unit", kind: "text", userOnly: true }, { id: "rate", label: "Rate", kind: "number", userOnly: true }, { id: "category", label: "Category", kind: "select", values: ["", ...RESOURCE_CATEGORIES] },
  ...
  async function applyBulkEdit() {
    ...
    if (mode === "ctc" && choice.userOnly && selected.some((row) => (row as CtcGridRow).resource_source)) { setError("Item, Description, Unit and Rate are read-only for rows added from Resource Rates. Remove those resource rows from the selection or choose another field."); return; }
    setSaving(true); setError("");
    try {
  ...
  async function saveView() {
    ...
    try { const saved = await saveProjectGridView(project.id, gridKey, viewName, { columnState: gridApi.getColumnState(), filterModel: gridApi.getFilterModel() as Record<string, unknown> }); setViews(await listProjectGridViews(project.id, gridKey)); setSelectedView(saved.id); setShowSaveView(false); setViewName(""); showNotice("View saved."); }
  ...
  async function deleteView() { if (selectedView === "Default") return; try { await deleteProjectGridView(selectedView); setViews(await listProjectGridViews(project.id, gridKey)); applyView("Default"); showNotice("View deleted."); } catch (requestError) { setError(gridViewErrorMessage(requestError)); } }
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 12000, background: "#f5f7fa", display: "flex", flexDirection: "column" }}>
    <header style={{ minHeight: 58, background: "#fff", borderBottom: "1px solid #dfe4ea", display: "flex", alignItems: "center", gap: 14, padding: "8px 14px" }}><button className="button secondary compact" onClick={onClose}>← Back</button><div style={{ minWidth: 0 }}><div style={{ fontSize: 16, fontWeight: 700 }}>{title}</div><div style={{ fontSize: 12, color: "#68707d" }}><strong>{costCode.cost_code_id}</strong> · {costCode.name}</div></div><div style={{ marginLeft: "auto", fontSize: 11, color: saving ? "#2563eb" : "#68707d" }}>{saving ? "Saving…" : "Auto-save enabled"}</div><button className="button secondary compact" onClick={onClose} aria-label="Close workspace">✕</button></header>
    ...
    <div className="enterprise-toolbar" style={{ flexWrap: "wrap", padding: "7px 12px", background: "#fff", borderBottom: "1px solid #e5e7eb", gap: 6 }}>
      ...
      <div style={{ display: "inline-flex", alignItems: "stretch" }} title="If a row or cell is selected, new rows are inserted directly below it. Otherwise they are added at the end."><button className="button primary" style={{ borderTopRightRadius: 0, borderBottomRightRadius: 0 }} disabled={loading || saving || (mode === "actual" && periods.length === 0)} onClick={() => void addRows()}>+ Add Row{addCount === 1 ? "" : "s"}</button><input type="number" min={1} max={100} value={addCount} onChange={(event) => setAddCount(Math.max(1, Math.min(100, Number(event.target.value) || 1)))} style={{ width: 54, border: "1px solid #2563eb", borderLeft: 0, borderRadius: "0 5px 5px 0", padding: "0 5px", fontSize: 12, textAlign: "center" }} aria-label="Number of rows to add"/></div>
      ...
      <button className="button secondary" disabled={!selectedCount || saving} onClick={() => { setBulkOpen(true); setBulkField(""); setBulkValue(""); }}>Bulk Edit{selectedCount ? ` (${selectedCount})` : ""}</button><button className="button danger" disabled={!selectedCount || saving} onClick={() => void deleteSelected()}>Delete{selectedCount ? ` (${selectedCount})` : ""}</button><button className="button secondary" disabled={!hasGroups} onClick={() => gridApi?.expandAll()}>Expand All</button><button className="button secondary" disabled={!hasGroups} onClick={() => gridApi?.collapseAll()}>Collapse All</button><button className="button secondary" onClick={exportRows}>⇩ Export</button><button className="button secondary" onClick={() => fileRef.current?.click()}>⇧ Import</button><input ref={fileRef} hidden type="file" accept=".xlsx,.xls" onChange={(event) => void chooseImport(event.target.files?.[0])}/><label className="status-filter"><span>View</span><select value={selectedView} onChange={(event) => applyView(event.target.value)}><option value="Default">Default</option>{views.map((view) => <option key={view.id} value={view.id}>{view.view_name}</option>)}</select></label><button className="button secondary" onClick={() => { setViewName(""); setShowSaveView(true); }}>Save View</button><button className="button secondary" disabled={selectedView === "Default"} onClick={() => void deleteView()}>Delete View</button><button className="button secondary" disabled={loading || saving} onClick={() => void refresh()}>↻ Refresh</button>
    </div>
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex" }}>
      <div style={{ flex: 1, minWidth: 0, padding: "8px 12px 10px" }}>
        {loading ? <div className="data-message" style={{ height: "100%" }}><span className="spinner"/>Loading {title} records…</div> : <AgGridProvider modules={[AllEnterpriseModule]} licenseKey={process.env.NEXT_PUBLIC_AG_GRID_LICENSE_KEY ?? ""}><div className="enterprise-grid-card" style={{ height: "100%", width: "100%", overflow: "hidden" }}><AgGridReact<RelatedGridRow> theme={gridTheme} rowData={rows} columnDefs={columns} defaultColDef={{ sortable: true, resizable: true, filter: true, minWidth: 70, enableRowGroup: true }} quickFilterText={search} rowSelection={{ mode: "multiRow" }} selectionColumnDef={{ pinned: "left", width: 42, maxWidth: 42, suppressHeaderMenuButton: true }} getRowId={(params) => params.data.id} onGridReady={(event) => { setGridApi(event.api); syncGroupState(event.api); }} onSelectionChanged={selectionChanged} onColumnRowGroupChanged={(event) => syncGroupState(event.api)} onCellValueChanged={(event) => mode === "actual" ? void actualChanged(event as CellValueChangedEvent<ActualGridRow>) : void ctcChanged(event as CellValueChangedEvent<CtcGridRow>)} rowGroupPanelShow="always" groupDisplayType="multipleColumns" groupTotalRow="bottom" grandTotalRow="pinnedBottom" groupSuppressBlankHeader animateRows tooltipShowDelay={250}/></div></AgGridProvider>}
      </div>
      {mode === "ctc" && resourcePaneOpen && <aside style={{ width: 410, flex: "0 0 410px", borderLeft: "1px solid #dfe4ea", background: "#fff", display: "flex", flexDirection: "column", minHeight: 0 }}>
        ...
        <div style={{ padding: 10, borderBottom: "1px solid #e5e7eb", display: "grid", gap: 8 }}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, background: "#f3f4f6", padding: 3, borderRadius: 6 }}><button onClick={() => { setResourceScope("enterprise"); setSelectedResources(new Set()); }} style={{ border: 0, borderRadius: 4, padding: "6px 8px", background: resourceScope === "enterprise" ? "white" : "transparent", boxShadow: resourceScope === "enterprise" ? "0 1px 3px rgba(0,0,0,.08)" : "none", fontWeight: 600, fontSize: 11.5 }}>Enterprise</button><button onClick={() => { setResourceScope("project"); setSelectedResources(new Set()); }} style={{ border: 0, borderRadius: 4, padding: "6px 8px", background: resourceScope === "project" ? "white" : "transparent", boxShadow: resourceScope === "project" ? "0 1px 3px rgba(0,0,0,.08)" : "none", fontWeight: 600, fontSize: 11.5 }}>Project</button></div><input value={resourceSearch} onChange={(event) => setResourceSearch(event.target.value)} placeholder="Search Resource ID, name, category…" style={{ width: "100%", height: 30, border: "1px solid #d1d5db", borderRadius: 5, padding: "0 8px", fontSize: 11.5 }}/></div>
        <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>{resourceOptions.length === 0 ? <div style={{ padding: 18, color: "#6b7280", fontSize: 12 }}>No active resources match this search.</div> : resourceOptions.map((resource) => { const checked = selectedResources.has(resource.key); return <label key={resource.key} style={{ display: "grid", gridTemplateColumns: "26px 1fr 72px", gap: 7, alignItems: "center", padding: "8px 10px", borderBottom: "1px solid #f0f2f5", cursor: "pointer", background: checked ? "#eff6ff" : "white" }}><input type="checkbox" checked={checked} onChange={() => setSelectedResources((current) => { const next = new Set(current); if (next.has(resource.key)) next.delete(resource.key); else next.add(resource.key); return next; })}/><span style={{ minWidth: 0 }}><strong style={{ display: "block", fontSize: 11.5 }}>{resource.resource_id}</strong><span style={{ display: "block", fontSize: 10.5, color: "#6b7280", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{resource.resource_name} · {resource.category} · {resource.unit}</span></span><span style={{ textAlign: "right", fontSize: 11.5, fontVariantNumeric: "tabular-nums" }}>{numberFormat(resource.rate, 4)}</span></label>; })}</div>
