...
export default function ProjectResourceRatesPage({ projectPublicId }: { projectPublicId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const rows = await listProjectResourceRates(currentProject.id);
  ...
  async function deleteSelected(ids: string[]) {
    ...
    try {
      await deactivateProjectResourceRates(ids);
      setSelectedIds([]);
  ...
  return <div className="enterprise-admin-page">
    ...
    <section className="enterprise-grid-card">
      <div className="enterprise-toolbar">
        <label className="enterprise-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search resources…" aria-label="Search project resource rates" /></label>
      ...
      {!error && !loading && rows.length === 0 && <div className="data-message"><strong>No resources found</strong><span>{resourceRates.length ? "Try changing the search or filters." : "Add or import the first resource for this project."}</span></div>}
      ...
      <div className="grid-footer"><span>{rows.length} of {resourceRates.length} resources · {selectedIds.length} selected</span><span>Resource ID must be unique within the project</span></div>
    </section>
...
function ResourceDrawer({ projectId, resource, onClose, onSaved }: { projectId: string; resource: ProjectResourceRate | null; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (resource) await updateProjectResourceRate(resource.id, input);
      else await createProjectResourceRate(projectId, input);
      onSaved(resource ? "Resource updated." : "Resource added.");
    } catch (requestError) {
      setFormError(projectResourceRateErrorMessage(requestError));
    } finally {
      setSaving(false);
    }
  }
