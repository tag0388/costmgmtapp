...
const GRID_KEY = "project-resource-rates";
...
export default function ProjectResourceRatesPage({ projectPublicId }: { projectPublicId: string }) {
  ...
  const [editing, setEditing] = useState<ProjectResourceRate | "new" | null>(null);
  const [gridApi, setGridApi] = useState<GridApi<ProjectResourceRate> | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(projectResourceRateErrorMessage(requestError));
    } finally {
      setLoading(false);
    }
  }, [projectPublicId]);
  ...
  async function onCellChanged(event: CellValueChangedEvent<ProjectResourceRate>) {
    ...
    setError("");
    try {
      let updated: ProjectResourceRate | null = null;
      if (colId === "resource_name") {
        ...
        updated = await updateProjectResourceRateFields(row.id, { resource_name: value });
      } else if (colId === "category") {
        updated = await updateProjectResourceRateFields(row.id, { category: row.category });
      } else if (colId === "rate") {
        ...
        updated = await updateProjectResourceRateFields(row.id, { rate: value });
      } else if (colId === "unit") {
        ...
        updated = await updateProjectResourceRateFields(row.id, { unit: value });
      } else if (colId.startsWith("free_text_")) {
        const value = String(row[colId as keyof ProjectResourceRate] ?? "").trim();
        if (value.length > 255) throw new Error("Extra text fields must be 255 characters or fewer.");
        updated = await updateProjectResourceRateFields(row.id, { [colId]: value || null });
      } else if (colId === "is_active") {
        updated = await updateProjectResourceRateFields(row.id, { is_active: row.is_active });
      }
      if (updated) {
  ...
  const columnDefs = useMemo<Array<ColDef<ProjectResourceRate> | ColGroupDef<ProjectResourceRate>>>(() => [
    {
    ...
    },
    {
      ...
      cellRenderer: (params: { data?: ProjectResourceRate }) => params.data ? (
  ...
  function onGridReady(event: GridReadyEvent<ProjectResourceRate>) {
  ...
  async function saveView() {
    ...
    try {
      await saveProjectGridView(project.id, GRID_KEY, viewName, {
  ...
  async function confirmDelete() {
    ...
    setError("");
    try {
      await deactivateProjectResourceRates(deleteIds);
      setDeleteIds(null);
      setSelectedIds([]);
  ...
  function exportResources() {
    ...
    exportExcel(`${project.project_code}-resource-rates`, "Resource Rates", excelRows.length ? excelRows : [template]);
  }
  ...
  async function runImport() {
    ...
    try {
      ...
      await importProjectResourceRates(project.id, incoming, replaceExisting, setProgress);
      ...
      setImportErrors([projectResourceRateErrorMessage(requestError)]);
    } finally {
      setImporting(false);
    }
  }
  ...
  return <div className="enterprise-admin-page">
    <div className="enterprise-page-title"><div><h2>Project Resource Rates</h2><p>Manage project resource IDs, categories, units and standard rates.</p></div></div>
    <section className="enterprise-grid-card">
      ...
      {!error && !loading && <AgGridProvider modules={[AllEnterpriseModule]} licenseKey={process.env.NEXT_PUBLIC_AG_GRID_LICENSE_KEY ?? ""}>
        <div style={{ height: "calc(100vh - 190px)", minHeight: 520, width: "100%" }}>
          <AgGridReact<ProjectResourceRate>
            theme={gridTheme}
            rowData={rows}
            ...
            onSelectionChanged={(event: SelectionChangedEvent<ProjectResourceRate>) => setSelectedIds(event.api.getSelectedRows().map((row) => row.id))}
    ...
    {editing && project && <ResourceDrawer projectId={project.id} resource={editing === "new" ? null : editing} onClose={() => setEditing(null)} onSaved={async (message) => { setEditing(null); showNotice(message); await refresh(); }}/>} 
    ...
    {importRows && <ExcelImportDialog title="Import Project Resource Rates" rows={importRows} columns={EXCEL_COLUMNS} errors={importErrors} replace={replaceExisting} setReplace={setReplaceExisting} importing={importing} progress={progress} onCancel={() => !importing && setImportRows(null)} onImport={() => void runImport()}/>} 
...
function ResourceDrawer({ projectId, resource, onClose, onSaved }: { projectId: string; resource: ProjectResourceRate | null; onClose: () => void; onSaved: (message: string) => void | Promise<void> }) {
  ...
  return <><button className="drawer-scrim" onClick={onClose} aria-label="Close resource editor" disabled={saving}/><aside className="admin-drawer">
    <header><div><span>Project Resource Rates</span><h2>{resource ? "Edit Resource" : "Add Resource"}</h2></div><button onClick={onClose} disabled={saving}>×</button></header>
