...
export async function createCostToCompleteDetails(projectId: string, inputRows: (Omit<CostToCompleteImportRow, "period_qty"> & { row_order?: number | null })[]) {
  ...
  const body = inputRows.map((row) => ({ project_id: projectId, ...row, row_order: row.row_order ?? null }));
  const rows = await supabaseRequest<CostToCompleteDetail[]>(`cost_to_complete_details?select=${encodeURIComponent(detailSelect)}`, {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify(body),
...
export async function clearCostToCompletePeriodQty(detailIds: string[], costPeriodIds: string[]) {
  if (!detailIds.length || !costPeriodIds.length) return;
  const batchSize = 100;
  const periods = costPeriodIds.map(encodeURIComponent).join(",");
  for (let index = 0; index < detailIds.length; index += batchSize) {
    const details = detailIds.slice(index, index + batchSize).map(encodeURIComponent).join(",");
