...
export type Project = {
  ...
  created_at: string;
  updated_at: string;
} & Record<ProjectAttributeKey, string | null>;
...
const attributeColumns = Array.from({ length: 20 }, (_, index) => `e_attribute_${String(index + 1).padStart(2, "0")}`);
const projectSelect = ["id", "public_id", "enterprise_id", "project_code", "name", "status", "created_by", "created_at", "updated_at", ...attributeColumns].join(",");

export function projectAttributeKey(attributeNumber: number): ProjectAttributeKey {
  return `e_attribute_${String(attributeNumber).padStart(2, "0")}`;
}
...
export function updateProjectAttributes(projectId: string, attributes: Record<string, string | null>) {
  return supabaseRequest<Project[]>(`projects?id=eq.${encodeURIComponent(projectId)}&select=${encodeURIComponent(projectSelect)}`, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
...
export async function updateProjectAttributesBulk(projectIds: string[], attributes: Record<string, string | null>) {
