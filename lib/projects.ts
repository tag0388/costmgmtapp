...
export type ProjectInput = { project_code: string; name: string; status: ProjectStatus };

const projectSelect = "id,public_id,enterprise_id,project_code,name,status,created_by,created_at";
...
export async function createProject(enterpriseId: string, input: ProjectInput) {
  const [project] = await supabaseRequest<Project[]>(`projects?select=${encodeURIComponent(projectSelect)}`, {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({ enterprise_id: enterpriseId, project_code: input.project_code, name: input.name, status: input.status }),
  });
  return project;
}
...
export function updateProject(id: string, input: ProjectInput) {
  return supabaseRequest(`projects?id=eq.${encodeURIComponent(id)}`, {
    ...
    body: JSON.stringify({ project_code: input.project_code, name: input.name, status: input.status }),
  });
}
...
export function projectErrorMessage(error: unknown) {
  if (error instanceof SupabaseRequestError && (error.code === "23505" || error.status === 409)) return "That project code is already in use for this enterprise.";
