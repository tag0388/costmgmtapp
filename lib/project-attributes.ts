...
export type AttributeValueInput = { id?: string; value_id: string; value_name: string };
...
export async function getOrCreateProjectAttributeSet(enterpriseId: string) {
  ...
  return supabaseRequest<ProjectAttributeSet[]>(`attribute_sets?select=${encodeURIComponent(setSelect)}`, {
    method: "POST",
...
export async function saveProjectAttributeDefinition(
  ...
  const definition = existing
    ? await supabaseRequest<ProjectAttributeDefinition[]>(`attribute_definitions?id=eq.${encodeURIComponent(existing.id)}&select=${encodeURIComponent(definitionSelect)}`, {
        method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify(payload),
      }).then((rows) => rows[0])
    : await supabaseRequest<ProjectAttributeDefinition[]>(`attribute_definitions?select=${encodeURIComponent(definitionSelect)}`, {
...
async function syncAttributeValues(definitionId: string, values: AttributeValueInput[], existing: AttributeValue[]) {
  const normalized = values.map((value) => ({ ...value, value_id: value.value_id.trim(), value_name: value.value_name.trim() })).filter((value) => value.value_id && value.value_name);
  ...
  for (let index = 0; index < normalized.length; index += 1) {
    ...
    const payload = { attribute_definition_id: definitionId, value_id: value.value_id, value_name: value.value_name, sort_order: index + 1, is_active: true, updated_at: new Date().toISOString() };
    if (value.id) await supabaseRequest(`attribute_values?id=eq.${encodeURIComponent(value.id)}`, { method: "PATCH", body: JSON.stringify(payload) });
    else await supabaseRequest("attribute_values", { method: "POST", body: JSON.stringify(payload) });
  }
}
