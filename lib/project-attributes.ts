...
export function projectAttributeColumn(slot: number): `e_attribute_${string}` { return `e_attribute_${String(slot).padStart(2, "0")}` as `e_attribute_${string}`; }
...
export async function ensureEnterpriseProjectAttributeSet(enterpriseId: string) {
  const existing = await getEnterpriseProjectAttributeSet(enterpriseId); if (existing) return existing;
  try {
    const rows = await supabaseRequest<AttributeSet[]>("attribute_sets?select=id,enterprise_id,project_id,scope,category,is_active", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ enterprise_id: enterpriseId, project_id: null, scope: "Enterprise", category: "Project", is_active: true }) });
    return rows[0];
  } catch (error) {
    if (error instanceof SupabaseRequestError && error.code === "23505") { const concurrent = await getEnterpriseProjectAttributeSet(enterpriseId); if (concurrent) return concurrent; }
    throw error;
  }
}

export async function listEnterpriseProjectAttributes(enterpriseId: string) {
  const set = await getEnterpriseProjectAttributeSet(enterpriseId); if (!set) return [] as ProjectAttributeDefinition[];
...
export async function saveEnterpriseProjectAttribute(enterpriseId: string, slot: number, input: ProjectAttributeInput, onProgress?: (done: number, total: number) => void, deleteMissing = false) {
  ...
  const removed = existingValues.filter((value) => !wantedIds.has(value.value_id.toLowerCase()));
  await Promise.all(removed.map((value) => supabaseRequest(`attribute_values?id=eq.${encodeURIComponent(value.id)}`, deleteMissing ? { method: "DELETE" } : { method: "PATCH", body: JSON.stringify({ is_active: false, updated_at: new Date().toISOString() }) })));

  let done = 0; onProgress?.(done, wanted.length);
  for (const value of wanted) {
    const match = existingValues.find((item) => item.value_id.toLowerCase() === value.value_id.toLowerCase());
    if (match && !removed.some((item) => item.id === match.id)) await supabaseRequest(`attribute_values?id=eq.${encodeURIComponent(match.id)}`, { method: "PATCH", body: JSON.stringify({ ...value, is_active: true, updated_at: new Date().toISOString() }) });
...
export function projectAttributeErrorMessage(error: unknown) {
  if (error instanceof SupabaseRequestError) { if (error.code === "23505") return "That attribute slot or Value ID already exists."; if (error.code === "23514") return "Check the attribute name, slot and value details."; return error.message; }
